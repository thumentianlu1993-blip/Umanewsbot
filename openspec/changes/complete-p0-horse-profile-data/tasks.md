...
- [x] 6.4.2 (integration) 将正式执行拆为 Phase A prepare-only 与 Phase B trusted release SHA 解锁：candidate 不可自签，JSON 单次冻结字节读取并拒绝 symlink，HORSE term 限定复用，completion run 仅关联 artifact 明确认领记录；真实 Phase A mapping/candidate 已冻结，修正 sync window 后仅信任 release manifest SHA `f101bccff1a7269ef0cee38e546e9d56b430ede2f1dbd17521e59a690526d0e9`。
