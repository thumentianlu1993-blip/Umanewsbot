...
### Decision 5: 归属采用“叙事中心 + 分层证据”，不按命中数量投票 <!-- adr: adr-005-evidence-hierarchy -->

归属证据分为：

1. `subject_origin`：标题明确作为叙事主体的马匹、骑师、练马师、马主、育马场、拍卖机构和赛马机构所属地区。
2. `event_location`：明确赛事、赛场及其举办地区；标题没有更强主体叙事时决定主地区，主体原属地作为相关地区。
3. `context_region`：标题/导语中的明确国家或地区上下文，只能佐证，不能单独压过赛事证据。
4. `source_fallback`：来源默认地区，仅在没有可信内容证据或证据冲突时使用。

标题和导语证据权重大于正文背景提及；来源 URL、来源备注、普通词术语和仅出现在历史履历/血统背景中的地名不参与主地区判定。地区选择不是“命中最多者胜出”：标题明确以 `Team Hong Kong`、某国练马师或主体行动/成果为叙事中心时，主体地区可作为主地区、赛事地区作为相关地区；没有这种强主体句式时，明确赛事地区优先。法国机构、法国育马或 Arqana 拍卖为文章主题且没有更强赛事中心时，法国可作为主地区。

单词型英文马名/赛事名与普通词或同名赛事冲突时只视为弱证据；必须由赛场、国家提示、完整多词实体或唯一导语上下文佐证。`other` 可保存澳洲、爱尔兰、沙特、迪拜等非五个运营地区的主/相关归属证据，但不新增生产频道、发布配额或独立 QQ 窗口。文章未提供的历史参赛地区不得仅为提高 Gold Set recall 自动补齐。
...
单审或部分抽样生成带 `provisional_single_review` 来源标记的校准集。有主地区的行进入指标分母，明确排除和未选择任何地区的行分别保留；原始自由文本与规范值同时审计。该模式不伪造第二审核人或裁决状态，但“只有一位审核人”本身不再构成 no-go；只有满足相同的总量、分地区、跨地区和质量门槛，并完成生产 shadow 验收后，才可进入仅新文章 enforce。若后续增加第二审核人，冲突仍必须裁决。

本批用户明确不再补第二审核人，固定以现有 159 条单审标签做首轮 Gold Set。`multiregion-v3` 在本地冻结文章与术语快照上达到主地区准确率 `98.11%`、相关地区 precision `100%`、recall `54.84%`、无依据变化 `1.89%`、过度扩散 `0%`；日本/香港/英国/美国主地区均 `100%`，法国 `90.91%`。首发覆盖门槛调整为有效样本 `>=150`、五个运营地区各 `>=10`、真实跨地区样本 `>=20`；相关地区采用“高 precision、允许低 recall”，precision `>=95%`、recall `>=50%`。当前 `159 / 最少地区法国11 / 跨地区24` 及全部质量指标均达标，可进入生产 shadow。Gold Set 后续按新增来源、规则改版、shadow 误判和运营争议持续追加，不以首次达标视为封版。

### Decision 8: 用版本化 gold set 与阈值决定生产资格 <!-- adr: adr-008-quality-thresholds -->

仓库保存脱敏/可复现的标注 fixture，首发至少 150 篇：五个运营地区各不少于 10 篇，跨地区样本不少于 20 篇，并包含赛事、海外远征、育马、拍卖、机构、普通地名和来源 fallback 反例。每条记录保存期望主地区、相关地区、允许 fallback 与判定说明。该数量是进入 shadow 的最低线，不是封版规模；后续持续扩充。

启用门槛：总体主地区准确率不低于 95%，单地区不低于 90%；相关地区 precision 不低于 95%、recall 不低于 50%；无依据改变主地区比例不高于 2%；自动产生超过两个相关地区的过度扩散率不高于 1%；人工锁定覆盖为 0。生产近期 dry-run 还必须人工抽检全部 `needs_review` 和所有主地区变化。precision、明显错标和过度扩散属于用户可感知安全门槛；recall 主要衡量漏标，达到 50% 即可首发，之后持续提高。线上 recall 波动只告警并阻止继续扩大灰度，不自动关闭已启用功能；precision 或扩散门槛失败才要求回退。

这些阈值优先保证错误地区少于漏掉少量相关地区，因为错误地区会影响网页频道、发布配额和 QQ 群。阈值与报告格式配置化，但降低生产门槛必须形成新决策记录。

只有人工确认且输入 SHA 匹配的样本进入分母；报告必须同时输出有效分母、缺失/漂移样本和 Wilson 置信区间，缺失或漂移导致总有效样本少于 150、任一运营地区少于 10 或跨地区样本少于 20 时直接 no-go。单审样本保留 `provisional_single_review` 来源事实；多人审核产生冲突时仍须裁决后才能进入分母。
...
**Choice：** 从现有 `_term_regions()` 的逐文章 ORM 查询改为可复用 `AttributionBatchContext`，一次预加载活跃地区术语、aliases、赛事证据和批次涉及的来源配置，并按语言/最长拉丁 token 或非拉丁 bigram 建候选索引；候选命中后仍调用原边界匹配器复核。gold set、72 小时 dry-run 和 commit 复用同一快照。250 篇 PostgreSQL 基准目标为不超过 30 条 SQL、30 秒、256 MiB RSS 增量；超标即 no-go。自然单篇流程可构造单元素 context，不引入常驻跨进程缓存。当前真实校准快照含 17,474 条术语、38,806 个候选和 17 个来源；修复来源 N+1 后五轮 250 篇 PostgreSQL 基准为 `5 SQL`、`1.66–2.14s`、RSS 增量约 `49 MiB`。
...
单一 attribution mode 控制计算/写入，独立 related-region query 开关控制展示与路由。上线顺序：代码部署且 mode=off、查询关闭；达到覆盖和质量门槛的单审或双审 Gold Set 允许进入 shadow；shadow 计算不写地区并至少观察 24 小时，人工检查全部主地区变化和 `needs_review`；验收通过后 enforce 只处理新文章且相关查询关闭；再为网页和测试 QQ 群开启相关地区查询；近期 72 小时 manifest 回填；最后扩大正式群。每一步保存窗口指标和样本，失败可退回前一步。
...
- 无阻断性产品问题。现有 159 条单审 Gold Set 已达到首发覆盖与质量门槛，可进入 shadow；后续仍需持续补充五地区长尾与跨地区样本，不能因覆盖增长缓慢而降低既定准确率、precision 或扩散门槛。
