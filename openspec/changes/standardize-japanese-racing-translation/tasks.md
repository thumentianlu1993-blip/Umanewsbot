...
- [x] 1.6 (integration) 已编写格式占位符按标题/正文字段成功恢复、元数据审计、首次遗漏后重试、最终遗漏失败以及既有马名/人物占位符组合顺序回归
- [x] 1.7 (application) 已编写同步强制重译副作用回归，证明已发布文章状态、发布时间、`manually_edited_fields` 标记、发布配额和 QQ delivery 不变，同时目标中文字段按显式 `--force` 更新
...
- [x] 2.3 (integration) 扩充日文翻译 prompt，要求翻译非占位符、非已接受专名的普通片假名，不影响未知马名原文保护
