...
- [x] 6.4.1 (integration) 新增独立 `apply_reviewed_p0_horse_completion` prepare/dry-run/commit 链：显式消费 50 行 profile mapping decisions，绑定 v3/authority/mapping/production snapshot SHA，真实零写入模拟全部 schema 与 action，单事务锁定后幂等写入并整批回滚；能力、测试和真实生产 Phase A prepare 已完成，formal dry-run 与 commit 待 Phase B。
- [x] 6.4.2 (integration) 将正式执行拆为 Phase A prepare-only 与 Phase B trusted release SHA 解锁：candidate 不可自签，JSON 单次冻结字节读取并拒绝 symlink，HORSE term 限定复用，completion run 仅关联 artifact 明确认领记录；真实 Phase A mapping/candidate 已冻结，日本在役马同步窗口由实时 JBIS 总数复核证据补齐，最终仅信任 release manifest SHA `74be2ce42f425bbd24794fb9573ee8b71348f40b0ed6fc0af8599b167c575153`。
...
- [x] 6.7 (operations) 每地区人工发布 1-2 匹完整资料马，验收公开索引、详情页、移动端、完整赛事履历、主胜鞍、关注入口、新闻 tag 和 no-network 边界。（2026-07-21 完成：五地区各 2 匹已发布生产，索引/详情/分页/主胜鞍/关注/中文名待补/no-network/新闻 tag 全部验收通过，移动端由用户生产复核确认）
