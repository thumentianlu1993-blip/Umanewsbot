...
`wpstud` 解析器会识别已缓存的马名、赛事、骑师和马场表格。赛事、骑师和马场页面通常是英文原文配中文译名，因此输出 `source_language=en`；WP Stud 仍是社区来源，候选会在 `notes` 标记 `source_tier=community` 和 `requires_review=true`。
