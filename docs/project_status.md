...
- 回归集中在 `Sale / York / Kelso / Jockey Club` 等普通单词或机构词被误当强赛事证据，以及 `JRHA`、世界排名上下文缺失。第三候选已按 precision 优先修复并增加反例测试；完整 `stable 1423` 项、相关组合 `199` 项、PostgreSQL 250 篇性能契约和 OpenSpec `29/29` 均通过。
- 第三候选提交为 `12f4d057`，两个独立上下文得到相同 AMD64 image ID `sha256:c4e0f1e56afc...a917b`，镜像内归属测试 `83` 项通过。该镜像尚未切生产。
