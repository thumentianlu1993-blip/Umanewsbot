...
- 法国新闻源扩展：`2026-07-07` 本地已实现 `expand-france-news-sources`，准备随本次上线。新增 `tdn_france_broad` 英文补充来源，真实只读探测 accepted 且已输出 HTTP `200` / 最终 URL；`at_the_races_france` 仍为 HTTP `403` access_limited/deferred。新增来源默认灰度关闭，需 `enabled=true` 且 `production_approved=true` 后才进入生产窗口；审计已能区分无新增、解析失败和门禁 blocker，probe 已支持多关键词来源部分失败时输出 `query_errors`，并与国际源抓取一起支持单篇详情解析失败跳过继续、全失败时标记 parse failed / 来源 failed。
- 英文术语门禁误挡修复：`2026-07-07` 本地已实现 `fix-english-term-gate-region-filter`，准备随本次上线。英文发布校验第一版改为同地区 + 全局术语范围，配置化高歧义英文词降级为 warning；未配置短词 / 全大写词仅在非核心命中时派生降级，同地区 / 全局高可信核心实体缺失仍阻断。新增近期误挡文章重处理命令和生产审计 `gate_issues` 摘要；生产上线后需继续验证香港、英国、美国最近窗口的 blocker、候选和公开数量。
