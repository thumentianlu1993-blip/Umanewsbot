## 2026-09-19 北京时间与赛前刷新本地实现完成（CI认证阻塞，未合并/部署）

## 2026-09-19 赛事修复发布进展

PR209已进入最终全量验证；北京时间展示、空参赛条件、出马表标题和定时动态字段修复已实现并经独立审核。完整CI发现的旧节拍测试已修正，77项相关回归通过。生产尚未切换；以current_state及该变更release.md为准。

独立分支`codex/plan-pre-race-refresh-20260919`已完成北京时间详情/列表排序、未知条件隐藏、出马表标题和绑定来源持续刷新。保持3h/1h/10min，无迁移，新增刷新开关默认false。主线程实现、固定只读代理审核，6项返修关闭；103项相关回归及76项历史/页面回归通过。历史北京日期未定者保留届次年份检索、待定组末尾，不猜赛时。最终联合168项及前后指纹一致、固定代理最终APPROVED；GitHub登录失效401，PR Linux CI待恢复登录后执行；本地整仓仅部分诊断，不能称全量通过。Racing Post HTTP406、生产来源绑定及额度核算仍是启用包验收项。未推送/合并/部署、未写生产；历史828/830/924继续暂缓。见[实施记录](changes/pre-race-display-refresh-20260919/implementation.md)和[发布边界](changes/pre-race-display-refresh-20260919/rollout.md)。
