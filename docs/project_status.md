...
- 2026-07-02 新建 OpenSpec change `revive-ranked-news-for-publish`：用于让未发布文章在普通来源升级为榜单来源时被唤醒，自动重试翻译失败/待翻译文章，重新评分低分或价值不足文章，并按唤醒时间重新进入发布窗口候选池；人工拒绝、撤回、重复 blocker 和硬门禁不绕过。当前 proposal、design、delta specs、tasks 已完成，严格校验和全量 OpenSpec 校验均通过，尚未实现。
