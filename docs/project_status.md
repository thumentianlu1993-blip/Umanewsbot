...
最后更新时间：`2026-07-04`
当前版本：`v0.0.1`（正式域名 HTTP 接入已修复，自动化运营 MVP、公开首页资讯流、抓取新鲜度修复、后台快速术语创建与当前稿术语应用、外部马名索引识别链路、榜单重点 QQ 推送、公开文章 ID URL、国际赛马资讯扩展、多地区生产窗口和术语种子数据准备均已部署生产）
...
- 赛事日历 / 年度赛事页 MVP：本地已实现 `RaceEvent` 产品层、公开 `/races/` 赛事日历、年度详情页、后台 `/admin/race-events/` 工作台、CSV 种子导入、候选资料写入/应用、新闻自动/手动关联和人工移除保护；当前尚未部署生产，第一版只覆盖 P0/P1 重点赛事，不建设马匹数据库或完整赛果库。
...
- 多地区术语库与外部马名索引：`2026-07-03` 生产只读核对显示，正式术语库和术语候选池仍主要是日文。`2026-07-04` 已正式导入第一批术语种子候选，生产 `TermEntry` 从 `2054` 增至 `2062`，`TermAlias` 从 `2057` 增至 `2068`；新增英文术语 `8` 条，覆盖香港马名、赛事、骑师、练马师、马场和固定表达，另有 `2` 条既有日文马名术语更新。随后已补写这 `10` 条的 `racing_region`，当前新增英文术语为 `hong_kong`，两条日文更新为 `japan`，旧存量日文术语仍大多缺地区。外部马名索引仍以日本 `netkeiba` 为主体，香港 HKJC 仍只有小样本英文/繁中别名；英国、法国、美国当前生产 `External*` 表无写入。当前应把多地区识别能力理解为“模型、导入命令和语言分流链路已具备，正式术语库开始补第一批英文/HK 候选”，不应理解为各地区术语库或外部马名库已补齐。
- 术语种子数据准备：OpenSpec change `prepare-termbase-seed-data` 已完成实现、验证和归档；本地首版已新增 `prepare_termbase_seed_data` 管理命令、`stable.services.termbase_seed` 服务层、HKJC/WP Stud fixture、操作文档和后台术语导入模板更新；内置 fixture smoke 可生成 `seed_candidates.csv`、`seed_conflicts.csv` 与 `summary.json`，候选主表严格兼容现有 `import_terms` 字段，中文目标译名统一简体化，第一版不直接入库，不做 HKJC `racecards` PDF/排位表全量抽取，默认输出到独立 runtime 目录。WP Stud 第一批全量审核候选已生成 `210` 条香港/海外来港马名候选，带 `racing_region=hong_kong` 版本已在生产 `import_terms --dry-run` 通过，结果为新增 `210`、错误 `0`；因 WP Stud 为社区来源，本批尚未正式写库，等待人工确认。HKJC 专用抽取已从真实页面打通小批路径：`selecthorse -> selecthorsebychar -> zh-hk horse detail`，支持 `--limit-horses` 控制马匹详情数；当前 HKJC 主审核文件已扩展为 `500` 条官方香港马名候选，`0` 冲突、`509` 次请求全部 `200`、`incomplete=false`，临时 SQLite `import_terms --dry-run` 为新增 `500`、错误 `0`。OpenSpec change `prepare-hkjc-overseas-termbase-seeds` 已完成本地实现和全部任务，新增 HKJC overseas Race Card 双语 fixture、海外马名/骑师/赛事名候选、精确 Race Card 参数、`source_evidence.json`、可解释跳过和渲染 fallback 失败可见性；本地 fixture smoke 为候选 `9` 条、冲突 `0`、`incomplete=false`、`dry_run_error_count=0`。经用户确认后执行的低上限 live dry-run 输出到 `runtime/termbase_seed/hkjc-overseas-live-smoke-20260704_174924/`，入口页请求 `1` 次且返回 `200`，候选 `0`、冲突 `0`、跳过 `0`、`dry_run_error_count=0`，但因直接 HTML 未暴露 Race Card 链接而记录 `render_fallback_unavailable`、`incomplete=true`；后续若要稳定批量抓取 HKJC overseas 候选，需要补浏览器渲染缓存或解析 HKJC 前端 API。本能力尚未部署生产，也未写正式术语库。
- 观察自动发布质量与自动化日志
