...
赛事区修复及两条身份修复包已实现，经固定子代理五轮代码审核通过；257 项核心与 7 项身份修复 PG 测试通过，另修复浏览器发现的详情状态不一致，增量模块 12 项 PG 通过，正在完成最终全量 CI 对照。历史79场只有冻结清单，尚未恢复赛果；JRA新功能默认关闭，尚未上线。以 [当前状态](current_state.md) 和 [发布包](changes/fix-race-section-gaps/rollout.md) 为准。
