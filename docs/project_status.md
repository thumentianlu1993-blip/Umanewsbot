...
最新生产状态：提交 `ce676998` 已部署。赛事历史主干能力和 P0 马资料补全基础代码已同时在线；P0 迁移 `0027` 已应用，21 条既有赛绩全部回填幂等键，网络补全与自动首次发布继续关闭。P0 来源同步仅完成 dry-run，尚未写入 21,596 条术语候选或 992 场重点赛事候选，下一步仍以五地区各 10 匹人工样本为准。多地区新闻归属迁移 `0023` 已应用，但两个归属开关继续保持关闭，当前公开新闻站、发布窗口和 QQ 仍按旧主地区逻辑运行。
