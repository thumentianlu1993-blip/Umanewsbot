...
## 2026-07-15 多地区归属 V3.2 已通过 Shadow 准入

- 原审核快照保守对账后 Gold 有效 `153/159`；主地区准确率 `98.04%`、相关 precision `100%`、recall `70.83%`、过度扩散 `0%`，机器结论合格。
- 第四轮生产 72 小时只读审计 run `#5` 覆盖 `525` 篇且范围完整，`21` 条主地区变化、`3` 条 `needs_review`、`49` 条人工清单已全部复核，未发现阻止 Shadow 的明确错标；报告 SHA `5f892441...e3c2`，manifest `6edd94e0...fe1e`。
- 未 commit 任何归属，生产 mode 与相关地区查询仍关闭；候选镜像和容器已清理，生产服务、队列、锁、事务与 healthz 正常，约 `8.12 GiB` 可用。
- 当前只批准下一阶段 Shadow，不批准 enforce。V3.2 已合入最新 `main@c4087e6c`；完整 `stable 1436` 项与镜像内组合 `192` 项通过，双构建 AMD64 image ID 一致为 `sha256:498a226f...c560f`。下一步等待独立生产部署窗口。

## 2026-07-15 多地区归属第三轮 no-go，V3.2 本地修复完成

- V3.1 第三轮生产 72 小时审计覆盖 `531` 篇，范围完整，`20` 条主地区变化、`5` 条 `needs_review`、缺失/锁/候选漂移均为 0。Gold 因最新正文修复只剩 `147` 条有效，主地区准确率 `96.60%`、相关 precision `93.75%`、recall `65.22%`，结论 no-go；生产仍为 off，Shadow 未开始。
- 已针对 `Killarney maiden`、`Irish Oaks` 嵌套短赛事名和日本马“凱旋門賞へ”三类真实错标完成 V3.2 修复，并让 Gold 报告输出具体问题 article IDs。完整 stable `1434` 项、交叉 `282` 项、PostgreSQL 250 篇性能、Django/迁移和 OpenSpec `29/29` 通过。
- V3.2 最终提交为 `a842f886`，两个纯 Git 独立上下文构建得到相同 AMD64 image `sha256:054af99d...6289`，镜像内归属/灰度/法国来源 `126` 项通过。
- 第三轮候选已清理并交还生产窗口，prod/rollback/三服务未变化，约 `8.12 GiB` 可用，队列、任务、锁和事务为空，healthz 正常。下一步是提交并双构建 V3.2，在新窗口保守续签 12 条 Gold SHA 后执行第四轮完整审计；通过前不启用 Shadow。
...
## 2026-07-14 多地区归属 V3 第二轮 no-go

- 第二轮生产只读报告覆盖 `591` 篇，范围完整，包含 `29` 条主地区变化和 `4` 条 `needs_review`；Gold 有效 `156` 条，主地区准确率 `91.67%`、相关 precision `100%`、recall `48%`，未达到 Shadow 门槛。
- 回归集中在 `Sale / York / Kelso / Jockey Club` 等普通单词或机构词被误当强赛事证据，以及 `JRHA`、世界排名和本地主体备战海外赛事上下文缺失。第三候选已按 precision 优先修复并增加反例测试；完整 `stable 1424` 项、相关组合 `200` 项、PostgreSQL 250 篇性能契约和 OpenSpec `29/29` 均通过。
- 过渡候选 `12f4d057 / sha256:c4e0f1e5...a917b` 缺少最后一条香港反例修复，禁止部署；最终提交必须重新双构建 AMD64 镜像。生产尚未切换。
- 最终第三候选为 `8c6262cd / sha256:413b7add...67ec`，独立双构建 ID 一致，镜像内归属 `84` 项、Django 和迁移检查通过。生产仍未切换。
- 生产功能仍为 `off`，相关地区查询关闭，Shadow 未开始。等待正文修复任务交还稳定生产快照后，必须重新生成第三轮完整报告并人工复核，不能复用前两轮结论。
