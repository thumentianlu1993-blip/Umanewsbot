...
- HKJC 日语 alias 合并与文章术语回填工具：`2026-07-07` 本地 change `hkjc-ja-alias-article-backfill` 已实现。新增 `merge_hkjc_ja_aliases` 用于把同中文目标的日语主术语安全并入 HKJC 英文概念并停用冗余日语主术语；新增 `backfill_article_terms` 用于对已发布文章中文字段执行可审计术语回填。两者默认 dry-run，正式 apply 必须绑定已审核 artifact 或显式过滤范围；文章回填保存完整 before/after 字段值，默认保护人工编辑字段，且不触发重翻译、改写、发布状态或 QQ 推送变化。
