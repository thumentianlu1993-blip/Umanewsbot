...
- 多地区术语库与外部马名索引：`2026-07-03` 生产只读核对显示，正式术语库和术语候选池仍主要是日文。`2026-07-04` 已正式导入第一批术语种子候选，生产 `TermEntry` 从 `2054` 增至 `2062`，`TermAlias` 从 `2057` 增至 `2068`；新增英文术语 `8` 条，覆盖香港马名、赛事、骑师、练马师、马场和固定表达，另有 `2` 条既有日文马名术语更新。外部马名索引仍以日本 `netkeiba` 为主体，香港 HKJC 仍只有小样本英文/繁中别名；英国、法国、美国当前生产 `External*` 表无写入。当前应把多地区识别能力理解为“模型、导入命令和语言分流链路已具备，正式术语库开始补第一批英文/HK 候选”，不应理解为各地区术语库或外部马名库已补齐。
...
- 2026-07-03 地区归属错配审计：现有 `NewsArticle.racing_region` 与新闻源地区完全一致，6598 篇中 0 篇偏离“按新闻源地区”。严格实体地区口径只覆盖 462 篇且均为日本文章，按用户提出的第一种/第二种逻辑未发现结构化错配；但生产实体地区数据不足，`TermEntry.racing_region` 目前全为空，英法美外部马名/赛事正式缓存未落库，因此该 0 只能视为下限。关键词粗扫有 1213 篇疑似跨地区提及，需后续做实体地区识别改造后才能给出可信错配数。
