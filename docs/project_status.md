...
- Gold 生成器与评估器已共用可配置的 `150/10/20` 默认门槛；合并 `origin/main@9d6dec34` 后组合专项 205 项、完整 `stable` 1321 项通过（1 项按设计跳过），159 条 Gold 仍为合格，OpenSpec strict/all 28/28 通过。
...
- 现有 Gold Set 以用户完成的 `159` 条单审标签固定校准，不再补第二审核人；因此始终是 `provisional_single_review`，不能用于生产 commit 或开启多地区开关。
