...
- [最小修复计划](changes/fix-race-section-gaps/PLAN.md)沿用现有同步、候选与 recovery 链路，先处理具体发布／身份问题及误导展示，再按固定名单补数据；不扩展为新来源平台或运营系统。当前仅完成计划，未实现／发布。
- 按用户补充，赛前第 4 天（含）开始持续检查赛时／出马表，按用户确认的临近程度三档频率每 3 小时／1 小时／10 分钟检查；日本增加限定官方赛前读取，替代初版仅人工补两场的范围。来源真实窗口、预算及写入衔接是实施验证项，频率与提前更新均尚未上线。
