...
- 后续历史赛事镜像曾以旧代码底座覆盖生产，造成已应用 `0029` 的数据库与旧应用不兼容并阻断 netkeiba 新稿。现已恢复为组合镜像 `sha256:383a36c1...c7b4`，新闻写入实测恢复，五地区页面和任务服务正常。下一优先级是观察至少 3 个自然窗口，并把组合镜像中的历史实现完整提交到 Git，消除“仓库 HEAD 与运行镜像内容不同”的发布风险。
