...
最新生产状态：提交 `f221c7df` 已部署。英文术语命中级上下文门禁、运行账本和高性能重处理已在线，迁移 `0028` 已应用，`web/worker/beat` 统一处于 `shadow`；旧门禁继续决定状态，11 个运行记录全部为 dry-run，尚无 commit。100 篇 PostgreSQL 基准为 7.53 秒、19 条 SQL、约 34.8 MiB RSS 增量，性能门槛通过；四地区小批结果已生成，至少观察 24 小时且人工抽检通过前不切 `enforce`。赛事历史主干能力和 P0 马资料补全基础代码继续在线，网络补全、P0 全量来源写入、多地区归属开关和自动首次发布仍保持关闭。
