...
# 五区赛事中文名导入预演与未来 apply 边界（2026-07-20）

1. 本地 `outputs/translate-race-names-20260719/unified-import-preview-20260719T201619Z/` 已因最终译名更正而失效，仅作历史审计；其中 manifest/dry-run/rollback 和 `apply_ready=true` 均不得用于正式写入。日本序号 64 最终值为“京成杯秋季赛”，必须重新生成并锁定新 bundle。
2. `unified-import-preview-20260719T212116Z` 至 `T020815Z` 均因连续审查发现的问题而失效；`T020815Z` 仍缺非动作源 Series 6019 的完整行 CAS，并允许含让赛标记的非 allowlist 独立中文名进入补充更新，且 supplemental Event 未校验 seriesKey。三项已修复，但生产 SSH banner 当前超时，尚未生成新候选；禁止复用旧 snapshot/bundle。恢复后必须生成全新时间戳目录，并重新记录日本输入、manifest、生产全字段快照、dry-run、rollback、Excel 与 bundle SHA。
3. 未来 apply 前必须重新核对五份输入 SHA、manifest 业务 SHA 和生产 before compare-and-swap；任一字段或 `updated_at` 漂移即停止，重新生成 dry-run。
4. 写前必须创建当前数据库 custom-format 备份，记录大小和 SHA-256，并独立通过 `pg_restore -l`。不得用 `rollback-before.json` 替代数据库备份。
5. apply 必须是串行原子事务，只更新 manifest 明列的 `RaceSeries.chinese_name`、`RaceEvent.chinese_name`，以及香港 Event `16446` 的 `race_series_id / series_key` 和 HistoricalRaceEventTarget `49052` 的 `race_series_id`；两者必须同步 CAS、同步改绑并通过各自系列/年份唯一性检查。Event `96` 只能按受审 before 从“京成杯秋季让赛”改为“京成杯秋季赛”；另 `219` 场原文回退 Event 只可按受审 series/region/before 和 lock 状态同步为对应系列中文名。不得修改 `original_name`、来源证据、公开状态、manual lock 或污染系列 6019。
6. 写前事务必须按 execution-plan 的 `eventScope` 锁定并完整行 CAS `1301` 个相关 RaceSeries，再锁定其当前完整 `8885` 场 Event，精确比对父行、子集合、系列 ID/key/地区和非动作行 SHA 后才可处理 `8883` 个动作；任何新增、删除、改绑或漂移均整批停止。非 allowlist 独立中文名不得进入动作。写后独立 verifier 必须重查同一完整父子集合，并核对 1300/8883 计数、香港唯一修正、Event `96` 精确新值、219 场 fallback 对齐、让赛字样为零和生产页面抽检；还须把 OperationLog 的八项 bundle/artifact SHA 与当前 index/metadata 逐项匹配。
7. 正式执行包的确定性归档 SHA 为 `faae9dc9…8e08`，内含 apply、独立 verifier、input-lock、normalized-input、manifest、production-before、dry-run、rollback-before、execution-metadata、execution-plan、artifact-index 和 bundle-index；bundle-index 原始 SHA 为 `25d4e47d…e5a15`、内容 SHA 为 `040d6f02…9fa8c`。归档必须来自最新成功 review 后的不可变提交；宿主、容器、verify-only、commit、verifier 必须复算并显式绑定上述 SHA，任何变化停止。对象级 rollback 还必须与原 apply 日志逐项匹配 bundle-index、rollback artifact 和 production-before SHA 后才允许锁定并写回业务对象。production-before 中 `34,910` 个 full row 和整体 `secondSha256` 必须可从落盘 JSON 独立重算；execution-plan 的每个 before row SHA、稳定字段 SHA、restore 值和完整 Event scope 必须与完整 rollback/snapshot 一致，metadata 记录的四个大文件原始 SHA 必须与 bundle index 相同，任一不符即停。
8. lowcost custom backup 必须由 Compose `db` 容器执行 `pg_dump -Fc`，先写宿主 `.incomplete`，成功后原子改名，验证 `0600`、非空、SHA、PostgreSQL 16 和容器内 `pg_restore -l`。写后失败首选受审对象级 after-CAS rollback；CAS 漂移不得强制覆盖，升级人工整库恢复决策。
9. 经 ssh 执行含 `docker compose exec` 的脚本时，必须先 `cat > /tmp/<script>.sh` 落成远程临时文件再 `bash /tmp/<script>.sh`：若用 `ssh 'bash -s' <<EOF` 经 stdin 传脚本，`docker compose exec` 会耗尽共享 stdin，bash 在 EOF 正常退出时触发 cleanup trap 误删 `.incomplete` 备份，且输出静默丢失。同理 `docker exec` 需要 stdin 时必须加 `-i`。（2026-07-21 实际发布中确认。）
10. 2026-07-21 本任务已正式写入生产：候选 `unified-import-preview-20260720T220245Z`、提交 `8e9dba57`、备份 `/opt/umanewsbot/backups/race-name-translation/pre-race-name-translation-20260721T080706Z.dump`（SHA `51a0f8ae…d1388`）、OperationLog `105230`。回滚首选同 bundle `--rollback-commit`（对象级 after-CAS），事故整库恢复用上述备份。证据全集见 `docs/changes/import-reviewed-race-name-translations/release_report.md`。
