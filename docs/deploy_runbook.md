...
2. V3.2 必须包含 `Killarney maiden`、`Irish Oaks`/`Oaks` 嵌套和日本马“凱旋門賞へ”真实反例，以及 Gold 问题 ID 诊断字段。完成提交后重新生成纯 Git archive，使用两个独立上下文构建相同 AMD64 image ID，并在镜像内通过 Django、migration 与归属专项；V3.1 镜像 `sha256:bf666bc...e802` 已从生产删除，不得复用。
   - 最终 revision `a842f88609b6282e05db91c6b7b5a7fec32e113b`，tree `3f4e1a2f76ae62476e4ed1a52eb459ca45ff2bcb`，source archive SHA-256 `c8ca119d718c9639d4ccc2f8d72497f58d5cb39e0ceeacf6c2ba8cd3bb5bc078`，双构建 AMD64 image ID `sha256:054af99d7ac6009dbc800f43ac7771ee41b117fd8e8a747057c0730a22dc6289`。镜像内 check、migration 和专项 `126/126` 通过（另 1 项环境跳过）。
...
3. 部署时保持 `MULTIREGION_RELATED_REGION_QUERIES_ENABLED=false`；同步设置 `MULTIREGION_ATTRIBUTION_MODE=shadow`、`MULTIREGION_ATTRIBUTION_ROLLOUT_STAGE=shadow`、`MULTIREGION_ATTRIBUTION_GOLD_VERSION=multiregion-gold-v1-20260713-v32-reconciled` 和 `MULTIREGION_ATTRIBUTION_GOLD_SNAPSHOT_SHA256=ff2a450aac170b846be43a99469ee1e6e3ef3c3adbd5ed3aaef203536da777be`。四项必须在 web/worker/beat 中读到一致值；不得执行归属 commit、72 小时回填、网页/群相关地区查询或 QQ 扩散。
