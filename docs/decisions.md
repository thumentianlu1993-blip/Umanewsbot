...
0079前向发布与JRA配置激活使用两个独立固定意图和备份，先验证关闭态兼容性，再打开新三开关；独立review分别核验合同、配置意图和故障恢复。见[发布证据](changes/multisource-race-enrollment/release-20260922/README.md)。

## 2026-09-22 0079迁移与来源激活分开绑定

采用独立0079 schema/catalog/发布合同，复用旧合同的纯文件安全helper，不修改旧代际常量或运行时替换global。先关闭新开关、保留v1准入发布。后续来源配置用独立before/target意图，不放宽迁移发布时常驻flags一致性检查；没有event-ID allowlist时不把场地route描述成三场精确发现范围。

## 2026-09-21 PR212 独立审核后的身份与CI约束

赛卡与赛果在末端共用马号槽位身份约束；同号异名需要受审event-local crosswalk，不以马号当稳定马匹ID。明确更正标记从parser的明确corrected语义产生，不从内容变化推断。首次结果开放下界与后续轮询间隔分离，精确赛时T+3前或date-only当地比赛日前不领取。

全量回归以当前PR固定base.sha和head.sha比较，产物提交必须相符；手动触发提供完整baseline_sha。过旧基线的失败集合可能掩盖相对当前主线的回归。历史固定迁移合同测试隔离至真实历史schema，新增迁移不放宽原生产guard。

## 2026-09-20 多来源登记的实现边界

采用独立v2绑定/政策与追加0079迁移，旧v1运行合同和摘要保留。赛事强身份支持跨语种，名称相似仅召回；马号属于位置标识，跨名马匹只经受审event-local crosswalk映射。结果完整性依赖独立核验名单，不能用HTML闭合证明。公开结果使用发布时授权，当前写权限自然到期不撤销历史，显式来源/身份撤销仍隐藏。

旧v1转换限定无已发布历史的固定清单、关闭态、真实代码SHA和锁后有效期；不自动转换既有已发布历史。来源proof未齐的地区保持关闭。见[实施记录](changes/multisource-race-enrollment/implementation.md)。

## 2026-09-20 多来源赛事登记与资料能力分离（方案已审核，待实施）

用户明确要求所有地区不依赖 TRA，任一来源命中可登记，后续同赛多源去重。新方案将“可信身份唯一命中现有公开赛事”作为登记门槛，来源是否具有赛时、出马表或赛果能力分别决定写入资格；沿用一个 event、一个 enrollment、一个同步 owner。按资料能力选择首个合法来源并保持粘滞，有界故障切换保留授权与审计。

跨语种名称、拼写、冠名只用于候选召回；自动挂接依赖已验证来源 ID、发生实例或受审届次，所有强证据必须一致，数据库唯一键和统一锁序阻止并发重复。无既有身份时用旧基线与新鲜官方真实链接链建立 A0 seed；裸 URL 或传入 event ID 不够。外部未知赛事及已有重复 event 保持待审，不自动建赛或合并。

这项新设计替代 2026-09-05 旧提案中“仅完整能力来源竞争整场纳管”的未来实现要求；旧 v1 生产登记继续按旧合同运行。新增 v2 只在实现、迁移和精确发布后生效。32 项验收合同、分地区 proof、默认关闭开关及前向恢复见[方案](changes/multisource-race-enrollment/PLAN.md)。不改写旧记录为已完成事实。

## 2026-09-19 长来源URL保持共享缓存长度合同

不放宽snapshot scope的128字符限制，也不改原来源URL；仅将超长URL映射为固定前缀+SHA256，合法短key维持不变，避免无关缓存失效。通过真实共享缓存链路集成用例覆盖生产140/154字符场景；自动失败保持旧卡与延迟提示，未人为伪造成功或调度时间。

## 2026-09-19 赛前刷新按生产来源合同启用

真实HTTP预演确认Sporting Life使用UTC日期，退赛后ride_count可仅计有效出走；在保留URL、race ID、完整名单和身份校验的前提下兼容这两种合同。NYRA生产403，因此取消491来源绑定变更，保留能提供明确退赛的原Sporting Life来源；赛事已结束时遵守赛后边界，不强制恢复赛前。

保持3h/1h/10min。本次发布将既有每来源/地区日请求上限192调整至512，日字节仍1GiB，覆盖同桶多场和按2MiB计的失败预留；这是全局每桶上限，不声称已覆盖所有来源或所有赛事。Racing Post406仍明确保留缺口。已授权上线通过既有0078协调器完成，状态、实际版本与自然周期证据见[发布记录](changes/pre-race-display-refresh-20260919/release.md)。

## 2026-09-19 北京日期排序与未知历史可发现性

用户批准实现后，公开时间采用同一北京日期合同；已知瞬时按北京年份筛选，无法确定北京日期的历史记录只用既有届次保留检索入口，归入待定末尾，不补造00:00。未知参赛条件隐藏，完整卡标题仅出马表。持续刷新沿用3h/1h/10min，以固定Beat槽推进；保留最后有效自动卡直到正式卡接管，跨来源缺赔率清空；新源开关默认关闭，真实来源/地区额度在精确启用包核算。无新增表、队列或预算平台。详见[实施记录](changes/pre-race-display-refresh-20260919/implementation.md)。

## 2026-09-18 归一化按来源合同兼容并正式启用

生产格式兼容和最终固定镜像只读预览已通过，用户明确授权公开仓库推送、合并和上线，因此启用归一化展示。单位仅依据显式语法或已核验官方来源字段合同，候选改值不得继承旧字段来源证明；不批量修改历史数据。局部回归限定受影响范围，完整对照由PR Linux CI承担。实际版本、验证和恢复边界见[发布记录](changes/normalize-race-information/release.md)。

## 2026-09-18 归一化发布验证范围与单位兼容

按用户反馈，局部改动默认仅做新增及受影响测试，完整对照留给PR的Linux CI，避免在本机重复整仓测试。正式启用前的生产聚合抽样暴露m单位缺口，不能把解析器输出待核实当完整业务验收，也不根据地区或数字大小自动补单位。先验证来源字段合同再补展示适配，不批量回写；现阶段不打开新开关。当前0078普通代码rollback禁用，恢复以展示开关关闭及受保护重载/前向修复为准。

## 2026-09-18 赛事归一化采用请求内严格展示合同

原始字段、旧归一化写入与统计合同保留；新增默认关闭的 `RACE_INFORMATION_NORMALIZED_DISPLAY_ENABLED` 统一所有结构化展示入口。含糊单位／等级冲突待核实，词库同层冲突保留原名，不从国家或数字大小猜测。只读盘点不带apply；同着与公开权限由现有证据决定。详见[实施记录](changes/normalize-race-information/implementation.md)。

## 2026-09-18 PR205 CI证据获取方式

仅本次固定run35306749487：已完成的候选/基线全量测试原始工件，经过SHA、head、attempt绑定、完整同算法差集和两位固定代理独立复核，可以满足已批准的“完整失败集合比较”要求。平台日志汇总仍运行中必须如实保留；不等同于未完成测试可通过，不伪造全绿，不绕过分支必需检查。应用/配置/资料写入范围保持原批准发布包，详见[验证记录](changes/complete-race-gaps-20260918/validation.md)。

## 2026-09-18 人工核验赛前卡复用候选展示

海外参考来源及NAR官方卡使用现有候选只读展示，禁止通用apply写canonical；正式来源完整卡接管或handoff后整份隐藏。参考与官方来源保留不同label与authority，source URL/cache SHA/manifest审计保留，原空赛时仅凭明确时区证据回填。此改动不新增自动来源框架，不承诺手工补录覆盖持续更新；频率3h/1h/10min不变。具体代码需PR205正常发布；已回填候选不等于已公开。

# 关键决策

## 2026-09-17 赛事区实施保持窄范围和正式性门禁

用户明确要求主线程测试先行、实现并由子代理反复审核。实现复用既有 task、snapshot lease、来源配额、候选、owner 与正式结果门禁；无新表/队列。JRA只公开经适配器校验的受限候选字段，通用apply拒绝其BASIC/RUNNERS，正式TRA赛卡成功才接管。真实Shift_JIS字节、截断拒绝、关闭后重新核验、URL身份绑定及NAR排除经回归验证。配置默认关闭，代码审核不代表生产启用。

批准计划中的公开语义保持两层：已有授权的暂定结果明细继续展示；正式冠军、完赛筛选和首页胜者须 confirmed 证据及原公开门禁。全量旧测试按此口径补齐正式结果夹具和正反断言，不通过隐藏所有暂定明细来消除失败。

## 2026-09-17 赛前资料提前至第 4 天检查，并提高每日频率

- 用户明确要求：赛前比赛时间、出马表等资料从赛前第 4 天（含）开始更新，并进一步提高每日信息检查频率；不再把提前更新全部留到后续阶段。
- [修复计划](changes/fix-race-section-gaps/PLAN.md)按举办地日期解释 D−4 起点，用户已将 D−4～D−2／D−1／比赛日三档频率确定为 3 小时／1 小时／10 分钟，取代初版 30／15／5 分钟建议。日历口径及限定 JRA 赛时补齐／名单候选展示仍是实现提案，不冒充用户逐项批准；实施需验证来源能力、请求预算和与现有 owner 的衔接。
- 保留既有完整来源纳管及 lifecycle／赛果权限边界。D−4 表示主动开始查找和更新已公布资料，不保证上游已公布完整出马表。本次仅更新计划，未修改运行配置或生产数据。

## 2026-09-07 将 0078 发布恢复限定为两个已知数据库状态

`fix-0078-recovery-contract` 采用已审核方案：当前候选只接收 0077→0078 升级和
0078→0078 同版本发布。两者均在首次停止服务前生成本次备份证明与不可覆盖的恢复意图，
由现有 one-shot 入口独占迁移；中断后使用原恢复入口、新锁和原始意图继续。
旧 0077 在途协议交给原固定控制镜像处理，不自动提升为新协议。普通代码回滚继续关闭。
为控制实现范围，协调逻辑集中在一个脚本和一个共享校验模块，不增加通用迁移框架。
这项决策不代表已部署；当前验证状态见 `docs/current_state.md`。
...
## 2026-09-19 新确认的赛事公开口径（待实施）

用户明确要求比赛日期/时间只按北京时间公开，日历排序亦统一该口径；无法确认的参赛条件为空；完整卡统一称“出马表”，人工/来源审核详情留后台。原始举办地时区、日期及来源仍用于采集身份、调度和审计，不因展示变更改写。赛前频率保持既定D−4起3h/1h/10min，是否更新成功以当前名单/退赛/赔率快照及真实页面为准，不以身份发现成功替代。具体待实施方案见`changes/pre-race-display-refresh-20260919/PLAN.md`。
