...
## 2026-07-14：普通单词赛事术语必须有地点语法，世界排名允许关联五地区

- 术语库可以保留与普通英文词同形的赛事或赛场名，但单词命中不能自动成为强地区证据。ASCII 单词赛场只有在标题或首段出现 `at/in/to/from <赛场>` 等明确地点语法时才参与赛事优先级；否则继续使用标题上下文、来源地区或进入人工复核。
- `Jockey Club` 单独出现优先解释为机构短语，不视为 `other` 赛事；更长且明确的正式赛事名仍可独立命中。`JRHA` 在标题中视为日本赛马产业上下文，避免普通词 `Sale` 把日本拍卖新闻改到其他地区。
- 本地来源明确报道本地参赛主体正在“备战/准备参加”海外赛事，且正文仍有本地地区证据时，主地区保留本地，海外目标赛事只进入相关地区。没有未来目标措辞或缺少本地正文证据时，仍遵守赛事中心优先，避免把真正的海外赛果/前瞻错留在来源地区。
- 世界排名文章按 Gold 明确口径使用 `other` 主地区，并关联日本、中国香港、英国、法国、美国五个运营地区。这是“全球榜单”特例，不受一般文章相关地区最多 3 个的防扩散规则限制；其他文章仍保持超过 3 个即 fail closed。
- 本轮改变推断语义，归属规则版本从 `multiregion-v3` 递增为 `multiregion-v3.1`；后续报告、manifest 和 applied 记录不得继续复用旧版本名。
- 旧 Gold 与真实人工复核冲突时以已确认的赛事中心规则为准，例如法国赛事赛果 `8089` 保持法国主地区。不得为了单个冻结标签抬高统计数字而恢复已确认的错标。
- 每轮生产正文变化都会改变 Gold 输入 SHA 和全量审计 manifest。只有正文 one-off 全部结束并交还稳定快照后，新的完整 72 小时 run 才具有上线判定效力。
