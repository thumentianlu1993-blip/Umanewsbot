...
## 2026-07-15：V3.2 第四轮通过后只进入 Shadow，不直接 enforce

1. 单审 Gold 可在来源、标题、正文语义和当前推断均稳定时保守续签。本轮从最初审核快照重算后有效 `153/159`，机器指标与 49 条生产人工清单均通过，因此允许进入 Shadow；6 条阻断记录继续退出分母，不以手改 SHA 或补造第二审核人解决。
2. Shadow 的批准范围仅为新代码在生产计算并记录归属结果，不修改文章现有主/相关地区，不开放网页、群查询、历史回填或正式 QQ。相关地区漏标只作为 recall 改进项；任何明确错标、锁/正文漂移、任务失败或运行态异常都中止升级。
3. Shadow 观察期至少 24 小时，从组合镜像部署、mode 实际设为 shadow 且健康验收完成时开始计算。观察期后必须用新文章和运行记录重新核对 precision、失败原因、耗时和锁状态；通过后才可单独决定“仅新文章 enforce”。
4. 第四轮审计候选基于较早主线，只能作为规则验收证据，不能直接覆盖最新主线。部署前必须合入 `main@c4087e6c` 的 historical runner 安全补丁，重新执行完整测试、迁移检查、OpenSpec 校验、可复现 AMD64 双构建与镜像内专项。

## 2026-07-15：第三轮 no-go 后使用 V3.2 收紧赛事语法并输出 Gold 问题 ID

1. 第三轮虽然主地区准确率为 `96.60%`、相关 recall 为 `65.22%`，但有效 Gold 仅 `147` 且相关 precision 为 `93.75%`；任一硬门槛失败都保持 no-go，不以 recall 提升抵消错标风险。
2. ASCII 单词赛场除 `at/in/to/from <赛场>` 外，只增加 `<赛场> maiden/race/fixture/meeting/stakes/card/course` 这组明确赛事搭配。不得恢复裸 `Sale/York/Kelso` 命中，也不得用宽泛邻词提高 recall。
3. 同一文本命中完整赛事名和其内部短赛事名时，只有短赛事词没有在文本其他位置独立出现才抑制短词；存在独立提及时两条证据都必须保留，避免隐藏真实多赛事冲突。
4. 日本来源标题以“凱旋門賞へ”等明确未来目标报道日本马或日本团队时，主地区保持日本、法国为相关地区；赛果、售票公告等既有赛事中心规则不变。
5. Gold 质量报告必须同时输出漂移、主地区错标、相关假阳性和相关漏标的 article IDs。指标用于 go/no-go，问题 ID 用于修复与复核；两者都不得因单审身份而省略。
6. V3.1 第三轮 run `#4` 只作为 no-go 证据，不得续用于批准 Shadow。规则语义变更后版本递增为 `multiregion-v3.2`，必须新建第四轮 72 小时 run 和 artifact。
