...
- 2025 Hampton Novices' Chase 按此规则处理：保留 `2025-01-11 / Warwick / ABANDONED` 为原赛程变更证据，以 `2025-01-19 / Windsor / WEIGHEDIN` 为年度实际赛事。正式字段为 Windsor、`3m53y`，出马和赛果以实际举办页为准。
- 日期 artifact 只修日期、距离和直接来源，不能把总账原定场地自动改成实际场地。日期 apply 后必须通过独立权威字段 artifact 校正 `racecourse`，字段更新导致 target SHA 变化后再重新导出和打包详情。
- 以后遇到同类证据，至少检查同赛事名、同届次、相邻日期、其他场地和主流结果库；只有排除改期/移师举办并取得独立取消证据后，才进入 `cancelled` 产品审核。
