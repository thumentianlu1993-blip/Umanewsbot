...
## 2026-09-20 赛事多来源登记目标链路（方案通过，未实施）

赛事自动化拟由“TRA 完整能力才可纳管”扩展为“任一已准入来源唯一命中即可登记，按资料能力持续更新”。多来源绑定归属同一赛事，强身份消除跨语种与拼写差异造成的重复；覆盖监测包含未登记赛事，赛后 7 天仍可补入完整正式结果。当前运行链路尚未切换，逐地区来源适配与验证未完成；见[修复方案](changes/multisource-race-enrollment/PLAN.md)与[当前状态](current_state.md)。

## 2026-09-18 赛前资料展示补齐

赛事详情已支持已核验赛前候选名单：JRA路径和人工核验候选复用现有存储，明确区分官方资料/参考资料；正式完整赛卡接管后隐藏候选。候选不经通用apply写入canonical。9场77条人工核验名单本次上线公开，既有3场JRA预览保持；D−4起3h/1h/10min频率不变。覆盖边界和剩余资料缺口见[验收清单](changes/complete-race-gaps-20260918/acceptance.md)。

# 项目总览

## 2026-09-17 赛事区更新链路（代码完成，未发布）

赛前检查从举办地 D−4（含）开始，按 3h / 1h / 10min 三档临近节奏复用现有同步、预算与任务链。新增限定 JRA 官方页面的候选展示路径：校验后展示参赛名单，受控补齐赛时；不写 canonical 参赛表，TRA 首张完整赛卡成功后接管。新 JRA 开关默认关闭。正式赛果仍须来源证据及现有公开权限，过期缺资料赛事不再显示赛前。

没有新增表、迁移或队列。代码状态、验证和生产数据边界以 [当前状态](current_state.md)、[验证记录](changes/fix-race-section-gaps/validation.md) 与 [发布包](changes/fix-race-section-gaps/rollout.md) 为准；不能把代码实现或缺口清单当作历史赛果已恢复。
...
0075 -> 0077；四服务随后恢复同一 image/revision，10 个赛事开关全 true、staging 写开关 false，队列
`0/0/7543`、资源与公网均通过。

France 2023 首批 5 个 provider stable ID 已完成生产 External staging：5 horses、60 去重 races、67 target
results、67 histories、5 source-display name variants 和 5 SUCCESS receipts。生产 dry-run 精确为
`create=210/update=0/skip=7/conflict=0`，单事务 apply 后独立 verifier 证明逐表增量、manifest/stable ID、
event 956、registry 与锁全部守恒；`HorseExternalIdentity` 仍为 0。该链只保存经 content-addressed TRA
profile/career/race/runner 证据重验的 External 数据，不创建其他 runner/父母实体，也不做 canonical identity、
HorseProfile、registry 或公网发布。后续跨语言身份桥接和更大批次继续作为独立阶段。
