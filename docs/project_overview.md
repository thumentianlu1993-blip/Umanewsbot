...
能力；本地候选进一步支持把爱尔兰、加拿大、阿联酋、沙特和澳大利亚作为五个独立新闻地区。
阿联酋和沙特可以在界面视觉上归入“中东”，但持久化、来源准入和灰度必须分开，不使用
`middle_east` 合并键。`0047` 扩展共享 `RacingRegion` 模型 choices；赛事与马匹运营表单
以及 Django `RaceEventAdmin` 仍只允许旧五区加 `other`，既保留旧 `other` 记录的可编辑性，
也不开放新五区结构化录入。实际抓取任务、自动选择器、公共 horse/race resolver 以及
historical、P0、race-live capability sets 仍锁在日本、中国香港、英国、法国、美国五区。
本任务没有提供新五区结构化数据抓取或生产能力。多地区常态生产遵循灰度原则：
...
- 新地区和新来源必须逐项通过技术解析、自动化许可与有效生产资格的显式准入；当前新增五区
  仅为默认关闭、尚未发布的本地候选。HRI、Woodbine、ERA 的自动化许可为 blocked，
  JCSA、Racing Victoria 为 unknown；受控技术复测也尚未形成 accepted artifact，因此当前
  没有 eligible 来源，不代表来源已上线或许可已批准。
