...
- PR #188 六模块候选 `61afbf59` 的 [Linux/PG16 CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34329843286) 已完成：4,943 项、31 failures、156 errors、20 skipped，173 个唯一失败 ID；相对生产候选消除 104 个原失败 ID，同时新增 3 条直接行锁 helper 测试的事务错误。原 105 条隔离级别错误全部消失，其中 100 项测试通过、5 项继续暴露外连接加锁错误；45 项发布合同、Django check、migration drift 和正式前后指纹通过。新增问题已通过给两个测试加显式事务修正，函数体及断言不变；当前候选 `cb829735` 经原 reviewer 增量复审通过，[新 CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34333661750) 待实际验证，不能提前称无新增失败。证据为本机 `runtime/fix-remaining-transaction-tests/ci-comparison-r2.json` 及 `delivery.json`。Windows 缓存试跑仍未到达主要断言；失败 apply 在回调注册前注入异常，未新增“已注册回调被丢弃”的覆盖声明。
- 外连接加锁应用修复已形成 [Draft PR #189](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/189)，候选 `936a7be4`：赛事与可空系列分开锁定，目标及非空系列保留原锁，覆盖首次关联、已有关联和撤销；身份与审批规则不变。新增独立连接行锁及无系列身份冲突回归，独立只读审核通过，[Linux/PG16 CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34332206795) 待完成。本机 24 项为 14 通过、5 failures、4 errors、1 skipped，未通过部分受文件校验/符号链接限制影响，不能当作 PostgreSQL 通过；具体记录见 PR 和 `runtime/fix-reconciliation-row-locks/delivery.json`。
- 性能修复已形成 [Draft PR #190](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/190)，候选 `93c7029a`：将原有缺席别名排除条件提前到筛选与排序之前，保留匹配语义，不增加缓存或框架。原 100 篇/2 万别名用例在本机实测 15.59 秒失败；修复后原术语模块 41 项合计 3.573 秒通过，包含原 10 秒及查询数断言。独立只读审核及 1,000 组隔离新旧结果对照通过，[Linux/PG16 CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34333456998) 待完成；本机证据在 `runtime/diagnose-term-performance/`、`runtime/fix-term-scan-performance/`。#189/#190 均未合并或部署，未执行历史数据动作。
