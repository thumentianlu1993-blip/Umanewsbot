...
- 第三候选规则版本递增为 `multiregion-v3.1`：ASCII 单词赛场只有在 `at/in/to/from <赛场>` 这类明确地点语法中才是强证据；`Jockey Club` 单独出现按机构短语处理；标题 `JRHA` 提供日本上下文；世界排名明确归属 `other` 并关联日本、中国香港、英国、法国、美国。对应反例均已固化为测试。
- 本地验证已通过：证据优先级 `35/35`，多地区 + 法国新鲜度 + 英文门禁 + 重处理 + 翻译恢复组合 `199/199`（另 1 项按环境跳过），完整 `stable 1423/1423`（另 7 项环境专项跳过）；真实 PostgreSQL 16 的 250 篇性能契约、Django check、迁移漂移、OpenSpec strict/all `29/29` 和 `git diff --check` 均通过。
- 第三候选源码提交为 `12f4d0579157f49f2bda8453397d192f42d89127`，tree `f5a7801e77811c736bcab5dfdc4f540bea7602df`，纯 Git source archive SHA-256 `5a127a646aaa37143590456832ba82e60b77d76dabda7b266f414226d319f6de`。两个独立本地上下文构建出相同 AMD64 image ID `sha256:c4e0f1e56afcad5975c7eaabec5f63781bb48d24074e348641c35af1801a917b`；镜像内 Django/migration 检查及归属 `83/83` 通过（另 1 项环境跳过；两份 Compose 静态契约按镜像设计只读挂载）。
