...
- 发布转换时钟/日历夹具修复已形成 [Draft PR #192](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/192)，候选 `9fef94fd`：只固定共享测试时钟并补齐 `local_date`，保留应用的实时到期拒绝和原断言；新增来源、策略、官方核验路由三种到期后拒绝回填时间的回归。真实 RED、原发布/日历及新回归在基类和继承类的 GREEN、独立只读复审已完成。三个模块本机 54 项为 23 pass、23 个测试产生 27 errors、8 PostgreSQL skipped；25 条错误受 Windows 0700 权限限制，另 2 条 slug 直接 update 错误在生产 Linux 基线也存在。Linux/PG16 CI 待核验，不预报修复数量或声称全绿；证据在 `runtime/fix-publication-test-clock/`，未合并或部署。
