...
- 用户要求写 plan 并避免过度设计。已在同一分支整理[修复计划](changes/fix-race-section-gaps/PLAN.md)：同日正式赛果核验回退、829 精确身份修复、104／105 的 JRA 候选补齐、过期状态／筛选与时间文案修正，以及现有流程内的历史回补。
- 交付收敛为一份代码 PR＋明确名单的数据修复包，预期不新增表／队列；日本本轮采用受控候选补齐，不声称建成长期 JRA 自动同步。保留既有来源归属、写入保护和公开门禁；接口正式性、未编号名单及后续纳管衔接需先验证。
- 仅更新计划与交接文档，未修改应用、调用生产或启动抓取／导入／部署；此前审计数量和运行版本仍是原时间点证据。尚未实施，不将计划写入已批准业务决策。
