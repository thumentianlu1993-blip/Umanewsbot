...
`2026-07-04` OpenSpec change `race-event-page-mvp` 已按已确认 Stitch 原型进入本地实现阶段，并完成赛事日历 / 年度赛事详情页 MVP 的代码侧闭环：新增 `RaceEvent` 产品层模型、别名、出马表、赛果、历史冠军、候选资料和 `ArticleRaceLink`；公开入口为 `/races/` 与 `/races/<year>/<slug>/`，文章详情页会展示已确认关联赛事；业务后台新增 `/admin/race-events/`，支持赛事列表筛选、详情维护、候选资料应用、手动关联/移除新闻和人工移除保护。管理命令新增 `import_race_events`、`fetch_race_event_candidates` 和 `research_live_race_fields`，样例 CSV 位于 `server/stable/data/race_events_seed_sample.csv`。本地 code review 后已修复后台赛事列表筛选翻页丢参问题，并补充回归测试。本能力仍处于本地实现与验证阶段，尚未部署生产；第一版不建设马匹数据库、完整赛果库、复杂赛事聚类或赛中实时进度。
...
`2026-07-04` WP Stud 第一批全量审核候选已生成但尚未正式导入。已从可直接访问的 WP Stud 页面缓存并转换编码，输出 `runtime/termbase_seed/wpstud-full-review-20260704/seed_candidates.csv`、`seed_candidates_with_region.csv`、`seed_conflicts.csv` 与 `summary.json`；候选共 `210` 条，冲突 `0` 条，全部为 `term_type=horse`、`source_language=ja`、`source_tier=community`、`requires_review=true`，中文译名已转为简体。带地区版本统一设置 `racing_region=hong_kong`，用于描述香港或海外来港赛马候选；生产已对 `/opt/umanewsbot/imports/wpstud_full_review_20260704_seed_candidates_with_region.csv` 执行 `import_terms --dry-run`，结果为总计 `210` 条、新增 `210` 条、更新 `0` 条、错误 `0` 条。由于 WP Stud 属于社区整理来源，本批仍停在人工审核/确认点，未写入正式 `TermEntry`。HKJC 真实页面此前可访问但通用解析器拿不到候选；本地已补 HKJC 专用抽取路径，从 `selecthorse` 发现字母页、从字母页拿 `horseid + 英文名`，再抓繁中马匹详情页对齐中文名，并新增 `--limit-horses` 控制小批马匹数。真实 HKJC smoke：`--source hkjc --allow-network --limit-pages 1 --limit-horses 3 --max-requests 10 --request-interval-seconds 0` 生成 `3` 条官方马名候选、冲突 `0`、请求 `5` 次且均 `200`、`incomplete=false`，样例包括 `AERIS NOVA -> 风再起时`、`AERODYNAMICS -> 友莹光`、`AWESOME FLUKE -> 非惟侥幸`；输出目录为 `runtime/termbase_seed/hkjc-live-smoke-20260704/`。该 smoke 只生成本地审核文件，未写生产正式术语。
...
`2026-07-04` 应用户要求扩展 HKJC 审核包，本地继续低频抓取 `500` 匹香港马官方译名，输出目录为 `runtime/termbase_seed/hkjc-formal-review-20260704_500horses/`，该目录应作为当前 HKJC 主审核文件，覆盖前一份 `100` 条小批。命令使用 `--limit-horses 500 --max-requests 560 --request-interval-seconds 1.5 --timeout-seconds 25`，结果为候选 `500` 条、冲突 `0` 条、请求 `509` 次且全部 `200`、`incomplete=false`。CSV 抽检显示 `500` 条均为唯一英文马名，全部为 `term_type=horse`、`source_language=en`、`racing_region=hong_kong`、`source_tier=official`、`requires_review=false`；样例包括 `A AMERIC TE SPECSO -> 有财有势`、`A TIME FOR US -> 开心孖宝`、`ABSOLUTE AWAKENED -> 活力精神`，末段覆盖到 `HYMNBOOK -> 北斗福星`。临时 SQLite 迁移库 `import_terms --dry-run` 通过：总计 `500` 条、新增 `500` 条、更新 `0` 条、错误 `0` 条。本批仍只生成审核文件，未导入生产正式术语库。
