...
- 历史迁移测试隔离为 [Draft PR #191](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/191)，候选 `721428a2`：历史测试使用临时私有 PostgreSQL schema / SQLite 内存库，真实执行迁移，避免回退当前不可逆 0078 测试库；不修改已发布迁移和原测试函数体。独立审核及本机 5 项通过。[Linux/PG16 CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34337262093) 为 4,945 项、37 failures、229 errors、20 skipped，264 个唯一失败 ID；28 条准备错误全部消失，16 个原测试中 10 个通过，另 6 个暴露后续旧合同/迁移叶断言失败，无新增失败 ID。新增两项隔离与异常清理回归无失败或跳过，45 项发布合同及正式前后指纹通过。六条后续断言仍需收口，不能称该组全绿；证据为 `runtime/fix-historical-migration-test-isolation/ci-comparison.json`。以上 PR 均未合并或部署。
