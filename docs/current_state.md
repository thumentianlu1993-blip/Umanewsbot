...
- 用户优先提出的 M2 赛时显示缺陷已定位并形成 [Draft PR #194](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/194)，候选 `51a46c37`。`2026-09-10 01:49 UTC` 生产只读审计确认 962/963 的完整赛时正常，但来源未单列当地时间，导致已算出的 `15:00/15:35` 被错误标为 `runtime_admission_closed`，字段仍为空。最小修复新增 12 行，继续要求来源合同、off_time 和当地时间开关均允许，保留人工锁及现有 apply/claim 逻辑；新增 5 项回归及独立审核通过。完整本地模块 77 项中 73 通过、4 条既有 Windows 文件系统环境错误；[Linux/PG16 CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34427582283) 运行中。尚未发布；之后需核验自然同步补齐及双域名页面。页面沿用英国当地 15:00/15:35，等于北京时间 22:00/22:35。证据与验证文档在该 PR 和本机 `runtime/fix-m2-local-start-time/`。
