...
- 后续独立候选：[Draft PR #195](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/195) `be54a04a` 修复官方发布测试的审计/到期夹具，并去掉临时赛果恢复中的可空外连接行锁，保留已经单独持有的锁及原权限；本机 31 项通过，独立复审通过，Linux/PG CI `34428158172` 尚待逐构件核验。[Draft PR #196](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/196) `6d19771f` 在按 control→event 顺序加锁后只读复查原先缺失的 control，遇并发新建则受控拒绝并要求重新预检，避免唯一约束异常，保留原锁顺序和原并发测试，新增实际 PG 等待交错回归。独立审核通过，CI `34429414869` 尚待核验；本机 45 项为 8 pass、6 failures、27 errors、4 skipped，失败来自既有 Windows 文件系统边界，两个 PG 用例跳过，不宣称并发已通过。两者均未合并/部署，也不包含在 #193 或 #194 发布包中。
