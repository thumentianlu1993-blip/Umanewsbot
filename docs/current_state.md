...
- 用户优先提出的 M2 赛时显示缺陷已形成 [Draft PR #194](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/194)，固定候选 `51a46c37`。生产只读审计确认 962/963 的完整赛时正常，但来源未单列当地时间，导致已算出的 `15:00/15:35` 被错误标为 `runtime_admission_closed`，字段仍为空。最小修复新增 12 行，继续要求来源合同、off_time 和当地时间开关均允许，保留人工锁及现有 apply/claim 逻辑。[Linux/PG16 CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34427582283) 已核验：受影响模块 77 项（含 5 项新增回归）全部无失败或跳过，45/45 发布合同、check、migration drift 及正式前后指纹通过；独立代码审核通过。全量 4,948 项、32 failures、257 errors、20 skipped，275 个唯一失败，旧 274 个均保留；额外一项为旧 full-cohort 查询数测试的授权夹具于 9 月 10 日 00:00 UTC 到期。测试和服务 blob 与生产相同，隔离原样复现到期前通过、到期点以 registry_runtime_expired 失败；CI 仍红，不称全量全绿或零新增失败。02:34 UTC 生产仍 `69955960`，服务/配置/迁移/队列正常，02:35 双域名实际时间字段仍为待定。精确发布包在本机 `runtime/fix-m2-local-start-time/pr194-release-package.md`，尚未获得 G2、未合并或部署。发布后需验证自然同步补齐一次及双域名英国当地 15:00/15:35（北京时间 22:00/22:35）；不强制抓取或手工回填。
...
- 夹具后续修复：[Draft PR #197](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/197) `98a4b55e` 只为原 full-cohort 查询数测试传入固定 now，本地两项通过，保留全部查询数断言；[Draft PR #198](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/198) `5a9e9914` 校正实时赛果测试时钟、模型保存、正向字段准入和现有持久挂载断言，本地 26 项辅助测试通过。两者独立只读审核无发现，应用和默认开关不变。#198 的 Windows/SQLite 辅助 runner 对固定测试凭据使用读取替身，不覆盖 POSIX 权限或 PostgreSQL；两候选正式 CI `34433191662`/`34433196286` 尚待逐构件核验，不能计入组合通过或生产恢复。用户已明确批准这两份验证文档及 #195/#196 简短 CI 说明的公开，四项均已上传；该授权不含合并或部署。
