...
## 2026-09-09 发布版本标记与赛事发现诊断（开发中，未部署）

- 镜像构建时写入固定版本文件，应用设置优先读取该文件，防止旧 `.env` 覆盖运行审计使用的版本标记；无镜像文件的源码运行保留环境变量回退。
- 赛事发现新增按地区/日期的响应摘要、逐级匹配数量和有界未匹配样本，帮助定位空响应及地区、日期、场地、名称不匹配。身份规则、请求次数和业务开关不变。
- 设计与验证范围见 [小型修复说明](changes/fix-release-discovery/README.md)。本条描述候选行为，不代表生产已更新。
