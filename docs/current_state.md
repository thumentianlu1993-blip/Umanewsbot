...
从 `origin/main@953ea626` 创建隔离分支 `codex/multisource-race-enrollment`，保留原 horse_data 工作区。依据用户要求，七地区统一任一已准入来源唯一命中即可登记，其他来源挂接同一 event；登记与资料能力分离，名称相似度仅召回候选，强身份和数据库唯一约束负责去重。补入冷启动证据链、故障切换、赛后 7 天发现及未登记覆盖监测。

独立只读 Agent 首轮提出 2 项 P1、1 项 P2；返修后第二轮 APPROVED，无未关闭 P0/P1/P2。详见[方案](changes/multisource-race-enrollment/PLAN.md)和[审核](changes/multisource-race-enrollment/REVIEW.md)。当前只有文档，32 项测试合同尚未执行；业务代码、迁移、逐地区来源 proof、生产修复均未完成。本阶段未写生产或变更开关，不把方案通过当作问题已修复。
...
- [PR #201](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/201) 已合并为 `7981439555bec747d462910506c56b08449ae621`；生产部署固定候选 `9a88243ffcf4d628a2ceaa528a66871d566fca82`，tree `d3f6a2c9c284183fa5ef844ee988591e847e55a3`，于 `2026-09-15T02:54:10.391232+00:00` 完成验证。四应用镜像 `sha256:e022147a9ddd4063e7ac35b5591fd0a118335b211b2c88da90d3fec0cdc1f986` 一致，Web healthy、两个worker响应、0078全图空迁移计划；配置、开关、持久挂载及DB/Redis/OneBot保持，Nginx已reload。旧race_live7543未消费。
- 专属备份、manifest/intent/完成凭据通过校验，发布锁/active pointer/限制恢复标记清理；956七组业务摘要保持，双域名755/756/757/956/962/963的赛果行数11/6/7/10/9/11全部通过。普通代码rollback继续禁用；中断按本次SHA绑定意图续跑，完成后问题优先前向修复。
- PR201整合#186–192、#195–200；固定Linux/PG16 CI 4,961项，消除230项旧失败，剩余45项、新增0项，受影响模块、P0 PG8项、发布专项45项通过。剩余45项保留原测试与证据，未将CI作业成功解释为全量测试通过。上述13个来源PR随祖先提交进入主线，旧#193仍为历史整合候选。
- M2：962/963已形成9/11行正式版本、公开页面及同赛果自然correction幂等证据；截至9月15日观察仍未证明上游明确正式确认标记及实际完赛后30分钟确认目标，整体不标记通过。原独立只读观察任务继续；本次部署不是M2业务验收。
- 统一交接入口为[后续工作路线图](future_work_roadmap.md)，本次详细[交接记录](changes/release-pr201-production/handoff.md)汇总9月6日至15日的实现、发布、测试、影响判断和保留项。
...
- 用户优先提出的 M2 赛时显示缺陷已形成 [PR #194](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/194)，固定候选 `51a46c37`。生产只读审计确认 962/963 的完整赛时正常，但来源未单列当地时间，导致已算出的 `15:00/15:35` 被错误标为 `runtime_admission_closed`，字段仍为空。最小修复新增 12 行，继续要求来源合同、off_time 和当地时间开关均允许，保留人工锁及现有 apply/claim 逻辑。[Linux/PG16 CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34427582283) 已核验：受影响模块 77 项（含 5 项新增回归）全部无失败或跳过，45/45 发布合同、check、migration drift 及正式前后指纹通过；独立代码审核通过。全量 4,948 项、32 failures、257 errors、20 skipped，275 个唯一失败，旧 274 个均保留；额外一项为旧 full-cohort 查询数测试的授权夹具于 9 月 10 日 00:00 UTC 到期。测试和服务 blob 与生产相同，隔离原样复现到期前通过、到期点以 registry_runtime_expired 失败；CI 仍红，不称全量全绿或零新增失败。02:34 UTC 生产仍 `69955960`，服务/配置/迁移/队列正常，02:35 双域名实际时间字段仍为待定。用户已批准固定发布包，#194 合并为 `ced88882152729c8e31a9cddb5fd83094e30d196`，生产部署固定候选 `51a46c379d39efcea75b7e96cff8cdb3b1dece77`，于 `2026-09-10T05:23:06.346895+00:00` 通过部署复验。四应用版本一致，Web healthy，两个 worker 响应，0078 空迁移计划；配置、开关和持久挂载保持，旧 `race_live=7543` 未消费。DB、Redis、OneBot、Nginx 容器保持，Nginx 已重载；956 七组业务摘要一致，双域名 755/756/757/956 分别 11/6/7/10 行。截至 `2026-09-10 05:35 UTC`，962/963 均已由自然同步补齐为英国当地 15:00/15:35（北京时间 22:00/22:35），双域名四页一致。原拒绝审计保留，同 observation 分别只新增一次 applied；两场后续相同来源内容的 race_time 同步均成功且未重复写入。本次赛时显示及重放验收通过；未强制抓取或手工回填，M2 正式赛果/公开/correction 整体验收仍未完成。
- 下述独立修复统一比较的旧生产基线为 PR #184 的 4,943 项、31 failures、257 errors、20 skipped，274 个唯一失败 ID；下述分支结果尚未成为主线或生产状态。5 个文档中的旧流程残留引用已修复，原检查器及 4 项合同测试、独立只读复审通过，形成 Draft PR #186；收口进度和历史 PR 分类也已完成复审，最终交付版本及证据随 PR 记录。检查器及其拒绝规则保持不变。
...
- PR #188 当前候选 `cb829735` 的 [Linux/PG16 CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34333661750) 已完成并核验：4,943 项、31 failures、153 errors、20 skipped，170 个唯一失败 ID；相对生产候选消除 104 个原失败 ID、无新增，上一轮 `61afbf59` 的 3 条直接行锁 helper 事务错误已清除。原 105 条隔离级别错误全部消失，其中 100 项通过，5 项仍暴露外连接加锁错误，由 PR #189 处理。45 项发布合同、Django check、migration drift 及正式前后指纹通过；原 reviewer 增量复审通过，两个直接 helper 函数体及断言不变。证据为 `runtime/fix-remaining-transaction-tests/ci-comparison-r3.json`。Windows 缓存试跑未到达主要断言；失败 apply 在回调注册前注入异常，未新增“已注册回调被丢弃”的覆盖声明。
- 外连接加锁应用修复为 [Draft PR #189](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/189)。原独立候选 `936a7be4` 的 [Linux/PG16 CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34332206795) 为 4,945 项、31 failures、241 errors、20 skipped，258 个唯一失败 ID；相对生产消除 16 个、无新增，相关模块 24 项无失败或跳过，45 项发布合同和正式前后指纹通过。应用修复分开锁定赛事与可空系列，身份和审批规则不变。合并模拟发现 #188/#189 的测试导入冲突，当前 `22ab56b8` 已以双父提交纳入 #188 的 `cb829735`，保留原 #189 三文件及 #188 另五文件原字节；原 reviewer 增量审核和逐文件 blob 核验通过。[组合 CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34340519048) 已核验：4,945 项、31 failures、143 errors、20 skipped，160 个唯一失败 ID，相对生产消除 114 个、无新增；关联模块 24 项无失败或跳过，45 项发布合同、Django check、migration drift 及正式前后指纹通过。该结果包含 #188，不能再与其修复数相加；依赖交付需保留 #188 的 merge commit 祖先关系。证据在 `runtime/fix-reconciliation-row-locks/` 和 `runtime/fix-reconciliation-row-locks-integrated/`。
- 性能修复为 [Draft PR #190](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/190)，候选 `93c7029a`：仅将原有缺席别名排除条件提前，保留匹配语义。原 100 篇/2 万别名用例本机 15.59 秒失败；修复后原术语模块 41 项合计 3.573 秒通过，保留原 10 秒及查询数断言。独立审核及 1,000 组新旧结果对照通过。[Linux/PG16 CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34333456998) 为 4,943 项、30 failures、257 errors、20 skipped，273 个唯一失败 ID；原性能失败消失、无新增，41 项术语测试无失败或跳过，45 项发布合同、Django check、migration drift 和正式前后指纹通过。证据在 `runtime/fix-term-scan-performance/ci-comparison.json`。
- 历史迁移测试隔离为 [Draft PR #191](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/191)，首轮候选 `721428a2`：历史测试使用临时私有 PostgreSQL schema / SQLite 内存库，真实执行迁移，避免回退当前不可逆 0078 测试库；不修改已发布迁移和原测试函数体。独立审核及本机 5 项通过。[Linux/PG16 CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34337262093) 为 4,945 项、37 failures、229 errors、20 skipped，264 个唯一失败 ID；28 条准备错误全部消失，16 个原测试中 10 个通过，另 6 个暴露后续旧合同/迁移叶断言失败，无新增失败 ID。新增两项隔离与异常清理回归无失败或跳过，45 项发布合同及正式前后指纹通过。首轮证据为 `runtime/fix-historical-migration-test-isolation/ci-comparison.json`。当前候选 `151e2e92` 已修复这 6 个后续用例及另一审计准备用例：按真实已应用迁移图验证叶节点，审计使用私有迁移库，四个旧世代正向发布用例映射为当前 0078 合同的拒绝及无写入验证，保留当前 0078 正向 PostgreSQL 回归和其余 22 个用例函数体。原 reviewer 增量审核通过，7 项前后映射有记录；[当前 CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34343151742) 已核验：4,945 项、31 failures、228 errors、20 skipped，257 个唯一失败 ID，相对生产消除 17 个、无新增；7 项映射后的源定义与实际结果均已核对，全部无失败或跳过，相关迁移/隔离模块无剩余失败或跳过。45 项发布合同、Django check、migration drift 及正式前后指纹通过。历史对照检查仅因既有 10 秒术语性能用例标红，该失败存在于生产基线并由 #190 单独修复，不能称全量全绿。证据在 `runtime/fix-migration-test-contracts/ci-comparison-r2.json`。以上 PR 均未合并或部署。
- 发布转换夹具及正式赛果加锁修复为 [Draft PR #192](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/192)。首轮 `9fef94fd` 的 [Linux/PG16 CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34345529390) 已核验：4,945 项、31 failures、223 errors、20 skipped，240 个唯一失败，相对生产消除 34 个、无新增。后续 slug 夹具改用模型 save 并保留全部 11 种漂移及原断言；`d68b4258` 的 [CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34348998560) 已核验：4,945 项、31 failures、221 errors、20 skipped，238 个唯一失败，相对生产消除 36 个、无新增；相关模块只剩发布审计和外连接加锁两条错误。两轮的 45 项发布合同、Django check、migration drift 和正式指纹均通过。当前 `de5fb540` 已最小修复这两条：补齐已发布测试夹具的审计，先锁 revision 再经非空关联锁 observation/source，保留原权限及三类锁；新增约束、空证据拒绝和 PG NOWAIT 锁回归。真实 RED、SQLite 2 pass/1 PG skip、原 reviewer 增量及最终记录复审通过；[当前候选 CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34351623147) 已逐构件核验：4,948 项、30 failures、219 errors、20 skipped，235 个唯一失败，相对生产少 39 个且无新增。其中 38 个为发布相关修复，另 1 个是未改动性能用例本轮通过，不能归功于 #192；其根因由 #190 修复。三个相关发布模块及新增三项真实 PG 回归无失败或跳过，45 项发布合同、check、migration drift 和正式前后指纹通过。共享测试时钟和日历日期修复保留，应用实时到期拒绝不变；证据在 `runtime/fix-publication-test-clock/`。
- [整合 Draft PR #193](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/193)，候选 `9ebdf657`、tree `b65fb4d5570d4be52978523adc3dc7286282c397`，用于验证 #186（`99fd1e5b`）、#187、#188、#189、#190、#191 一起运行的结果，不包含 #192 或 #194。27 个文件逐 blob 与已审核来源一致，唯一交叠测试文件保留 #189 最终字节；独立只读组合审核通过。现有 [Linux/PG16 CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34349427487) 及全部构件已核验：4,947 项、30 failures、83 errors、20 skipped，111 个唯一失败，相对生产实际消除 163 个且无新增；45 项发布合同、Django check、migration drift 及正式前后指纹通过。相关模块与七个映射后的迁移用例无失败或跳过，不把各分支修复数量相加。该验证 PR 没有合并或关闭来源 PR，也未部署；后续进度文档增量与这份固定测试快照分开识别。证据在 `runtime/closeout-batch1-integration/`。
- 后续独立候选：[Draft PR #195](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/195) `be54a04a` 修复官方发布夹具及临时赛果恢复行锁；[CI 34428158172](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34428158172) 已逐构件核验，相关模块 31 项、发布专项 45 项通过，正式 Linux 指纹一致。[Draft PR #196](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/196) `6d19771f` 修复纳管加锁期间并发新建 control 的受控拒绝；[CI 34429414869](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34429414869) 原、新两项 PostgreSQL 并发测试通过，相关模块 43 pass/2 条 macOS 条件 skip，发布专项 45 项通过，正式 Linux 指纹一致。两者独立审核通过，但全量仍有失败，不能称全绿；尚未合并/部署，也不包含在 #193 或 #194 发布包中。
- 夹具后续修复：#197 `98a4b55e`、#198 `5a9e9914`、[Draft PR #199](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/199) `784198a2` 的固定 Linux/PG16 构件已核验 SHA 与候选。#197 两项查询数合同通过；#198 26 项相关测试通过，12 个旧失败 ID 消失；#199 两模块 30 项及原有退赛/NR 两项保护测试通过，两个旧失败消失。三者各有 45 项发布专项通过，正式 Linux 前后指纹一致。各自全量仍为 274/263/273 个唯一失败；#198/#199 额外一项是未修改的 full-cohort 到期夹具，由 #197 单独修复。#198 最后 comparison job 已于 9 月 10 日核验完成，四个作业均为 SUCCESS；全量旧失败仍保留。这些候选独立审核通过、公开上传完成，但未合并/部署，也未加入 #193 组合；不能累计为组合结果。Windows 原始环境失败及辅助替身边界继续保留在各验证记录。
...
- 0078 发布恢复、956 公开读取、镜像版本标记和发现诊断已交付，不再作为待实现工单；截至 `2026-09-10 01:40 UTC` 的独立观察已补存自然诊断：490 为 `response_empty`；766 随 tomorrow/today 窗口出现 `course_not_found` 与 `local_date_not_found`。这些分桶尚不足以判定实现缺陷或放宽身份规则，完整归因仍待证据核对。7 个历史 open PR 已完成第一轮只读分类，详见路线图 §0.1；#45 的多文章曝光回填缺陷仍存在，处置需单独判断，不能视为已修复。没有关闭 PR、执行历史回填或删除工作树。
...
- 镜像版本文件修复和有界赛事发现诊断已上线。Web、普通 worker、Beat、v2 worker 的镜像标记和 Django 有效设置均为 `69955960`，原 `.env` 字节及业务开关保持不变。Docker 原始环境变量仍可能是旧值，不能据此判断应用运行版本；核验方法及恢复凭据见 [部署运行手册](deploy_runbook.md)。
- `2026-09-09 07:12 UTC` 只读核验：Web healthy，两类 worker 均响应，四应用无重启/OOM；数据库完整 0078，全图迁移计划为空；普通/v2 队列为 0。旧 `race_live=7543` 保持隔离，未启动旧 worker。DB、Redis、OneBot、Nginx 容器未重建，Nginx 已验证并重载。
- 双域名的 755/756/757/956 赛果分别显示 11/6/7/10 行；956 的 event、results、enrollment、revision、items、observation、publication 发布前后摘要完全相同。此次没有补跑历史新闻稿件、强制赛事抓取或修复生产业务记录。
- 验证：本地相关 17 项、实际隔离镜像 13 项及 Linux/PostgreSQL 16 发布专项 45 项通过；独立复审无待处理发现，最终内容与复审 tree 相同。Django check、migration drift 和 Linux 原生前后指纹通过。[最终 CI](https://github.com/thumentianlu1993-blip/Umanewsbot/actions/runs/34319906713) 全量 4,943 项仍有 31 failures、257 errors、20 skipped，274 个失败 ID 与前生产候选 `31bf6095` 完全相同；相对同环境历史基线也无新增失败。本次直接相关模块无失败或跳过，工作流四个 job 成功不代表历史全量测试全部通过。
- 补记前次交付：[PR #183](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/183) 已合并为 `41a4c745`，候选 `31bf6095` 于 `2026-09-09 01:54 UTC` 上线，修复 956 遗留发布状态造成的赛果不可见问题；本次再次确认其 10 条赛果正常公开。下方早期“未部署”条目是当时的历史记录。
- M2 尚未完成整体验收。此前 `2026-09-09 06:15 UTC` 的只读证据已确认自然发现的 962/963 建立身份、进入报名并生成赛卡，分别为 9/11 匹、revision 19/20；计划开赛为 `2026-09-10 14:00/14:35 UTC`（北京时间 22:00/22:35），仍需观察正式赛果、公开展示及自然纠错。不能再将“新赛事发现无命中”概括为当前状态，也不能把旧 956 公开恢复等同于 M2 全链路验收。
- 每 15 分钟的 `umanews-m2` 只读观察已转入独立任务 `Umanews M2 只读验收观察`（`01a084d7-ad01-7130-b3a4-8197b3b9bee3`），状态不变时保持安静；证据目录为本机 `runtime/m2_watch_20260909/`。后续观察按部署版本分段，继续从该任务获取更新。490/766 的具体未匹配原因仍待自然发现结果中的诊断字段，不为本次验证额外调用来源。
- 本次发布摘要及复核证据：服务器 `/opt/umanews-release-69955960-PR184-20260909/verification.json`、`deploy-result.json`；本机 `runtime/fix-release-discovery/` 下同名文件、`pr184-ci-comparison.json` 和 `pr184-release-package.md`。GitHub 已保存代码和交接文档；凭据、配置文件、数据库备份不进仓库。

## 2026-09-08 0078 发布前核验与开关兼容修正（未部署）

- PR #181 已合并为 `51c0cf2377a00002c87011aad17c4d65a90b9fe7`，本机 SSH 已连通生产。只读核验发现生产应用仍为 `ca6e9d06`，数据库完整 0078、迁移计划为空，十项 RACE_DATA_SYNC 开关实际为 true。
- 原 0078 实现把临时 admission 容器要求的全 false 当作常驻恢复值，无法发布并保留线上配置。小型补丁 [PR #182](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/182) 从实际 Compose 冻结原值，仅对临时 schema/preflight/release-task 注入十四项 false，并在首次停服前和恢复时核对真实容器开关。全关闭及五类活动为零的门禁不变。
- 生产实现 `70d6bccf` 已通过独立只读审核与 45 项 Linux/PG16 专项（零跳过）。全量 4931 项相对上一候选无新增失败 ID，但本轮相对旧基线的 comparison 仍因既有 10 秒性能阈值波动失败；不能称 CI 全绿。最终扫描测试修正及证据边界见 [验证记录](changes/fix-0078-live-writer-flags/validation.md)。
- 网站健康、新闻首页、赛事和马匹页只读请求均为 200；生产真实 Compose 解析候选配置确认五个应用服务开关一致，原 `.env` SHA 未变。本次未清理遗留锁、备份、构建生产镜像、停服、迁移或写入生产业务数据。
- 发布范围仍为 0078→0078 空计划、保留当前业务开关、单发布备份/意图和原服务恢复。普通代码 rollback 未开放；实际发布与新 PR/commit 绑定，按根 `AGENTS.md` 执行。

## 2026-09-07 0078 发布与恢复合同修复（实现与验证完成，未合并/部署）

- 分支 `codex/fix-0078-recovery-contract`，基线 `a88bcbf6`；方案经独立复审通过。
- 修复目标是让 exact0078 校验、0077→0078 升级、同版本发布和失败续跑使用一致合同。
  普通代码 rollback 继续禁用；历史“恢复生产回滚能力”的描述不等于开放 rollback。
- 已接通同发布备份证明、首次停服前的持久意图、关闭态 handoff、单一迁移 owner
  和服务恢复。最终代码 `966e3455` 经独立测试与同上下文复审：39 项 Linux/PG16 专项通过，
  全量 candidate 4925 项相对固定 baseline 4886 项无新增失败；历史失败及测试替身边界
  见 [验证记录](changes/fix-0078-recovery-contract/validation.md)。
- 交付为 [Draft PR #181](https://github.com/thumentianlu1993-blip/Umanewsbot/pull/181)；
  已发布迁移文件不变，后续实际镜像/Compose、队列和连接切换验收属于精确发布包。
- 本轮没有生产操作；下方生产数据均为原日期的交接事实，不能视为本轮实时核验。
