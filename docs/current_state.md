...
## 2026-09-19 赛事展示与赛前刷新发布中（尚未上线）

用户已授权上线；PR209已推送，GitHub connector与SSH可用，早前CLI401不再是阻塞。首完整CI5125项对生产基线新增1项旧Beat节拍断言；测试已按07/17节拍修正并补充下一轮公平/缓存复用验证，77项相关回归通过，原reviewer APPROVED，等待最终提交完整CI。真实来源预演另修复美国UTC跨日及退赛后有效计数合同。服务器仅准备独立候选目录/镜像，当前运行仍为433de627；新flag与512请求额度尚未在运行服务生效。491沿用既有SL来源，取消NYRA绑定写入；RP406仍为覆盖缺口，历史828/830/924不动。详见docs/changes/pre-race-display-refresh-20260919/release.md。
