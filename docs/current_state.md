...
- 生产运行验证：
  ...
  - `2026-07-03` 地区归属错配只读审计：当前 `NewsArticle.racing_region` 与 `source_config.racing_region` 完全一致，`6598` 篇文章中 `0` 篇偏离“按新闻源地区”的现有逻辑。严格按有地区字段的实体（`ExternalHorseAlias` / 非空 `TermEntry.racing_region`）推断时，可覆盖 `462` 篇文章，且全部为日本文章；按用户提出的“第一种单地区逻辑”和“第二种多地区逻辑”均未发现结构化错配。但该结果只能作为下限：生产 `TermEntry` 的马/赛事/骑手地区目前全部为空（马 `1884`、赛事 `153`、骑手 `2`），`MajorRaceEvent` 为空，外部马名/赛事正式缓存只有日本和极少香港，没有英法美实体地区，因此系统无法可靠判断英文新闻中提到的日本 / 英国 / 法国 / 美国马、骑手或赛事。补充关键词粗扫发现 `1213` 篇疑似跨地区提及，其中 `2026-06-30` 以后 `231` 篇、`2026-07-02` `60` 篇，但噪声较高，只能作为后续补实体地区识别的线索。
