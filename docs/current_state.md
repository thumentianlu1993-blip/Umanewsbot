...
- 用户补充明确赛前第 4 天（含）开始、提高每日检查频率。计划按举办地日期从 D−4 当地 00:00 纳入主动更新，建议 D−4～D−2／D−1／比赛日分别每 30／15／5 分钟检查，缺赛时／来源身份也进入发现；具体频率需结合现有预算验证。
- 此补充取代初版“日本仅人工补齐、提前自动更新留待后续”的范围：计划增加限定 JRA 赛前页面适配器及未纳管赛事的受控赛时／名单预填，104／105 用作样本；自动 owner 接管后停止预填直接写入。此写入方案尚待实现验证，不代表已批准变更原来源权限。
- 交付仍为一份代码 PR＋明确名单的数据修复包，预期不新增表／队列。保留来源归属、人工锁和公开门禁；正式性、名单更新／纳管衔接与提频容量需先验证。
- 本次仅更新计划与交接文档，未修改应用、调用生产或启动抓取／导入／部署；此前审计数量和运行版本仍是原时间点证据。用户明确的提前窗口及提频方向记入 decisions，具体实现仍是待审阅计划。
