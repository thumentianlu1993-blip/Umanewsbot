...
- #195 `be54a04a` 与 #196 `6d19771f` 已形成独立 Draft，均完成只读审核；前者本机 31 项通过，后者受 Windows 文件系统限制，原/新增真实 PG 并发用例本机均跳过。CI `34428158172`/`34429414869` 尚待逐构件核验，不能提前计入通过或减少失败数。纳管并发修复保留 control→event 锁顺序，发现原先缺失的 control 在等待期间出现则整事务受控拒绝，正常重试保持幂等。
