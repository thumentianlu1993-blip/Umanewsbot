...
## 第 4 轮（2026-09-27，同一独立审阅者）：APPROVED

N4/N5/N6/N1 逐条复核确认真正闭合：3.1 的 digest 闭集与运行态校验链逐点对齐（admission.py:458-463、control.py:242/248/2816-2820/2864、enrollment.py:1276-1386/1486-1487/1617），并补充确认闭集无遗漏（`source_registry_digest`/`event_snapshot_sha256`/`identity_keys_sha256`/`enrollment.manifest_sha256` 均正确不在更新项内）。v4 未引入新阻塞矛盾。

附 3 条不阻塞措辞建议（已随 v4 一并落实，审阅者说明无需再走一轮）：

1. 目标/完成标准/任务 6 的"103–107"统一改为"预检时实际 enrolled 集合（预期 103–105 或 103–107）"；
2. 3.2 残留的"绑定该新 commit"同步为"归一 PR 与换绑 PR 均部署之后"；
3. 回滚语义补写：恢复 v1 policy 后已换绑登记自然刷新停止（已公开内容不受影响），回滚只用于中止扩版。

四轮汇总：R1 发现 P0-1 digest 漂移断链（计划原本完全未处理）→ R2 证实"9/7 同款轮换"对 v2 登记不可用 → R3 确认新增换绑路径形态正确并补全字段闭集 → R4 APPROVED。

实施前不可豁免核对项（历次审阅一致声明）：服务器运行态（`.env`、锁、`release_0079` 模块）；v1 `proof_digest` 生成方式（任务 3）；预检时实际登记集合（任务 5）；新换绑 PR 与新激活脚本各自独立审阅。

## 第 5 轮（2026-09-27，同一独立审阅者）：REVISE

核心变化确认：分层 proof 纳入论证成立（venue 映射错误只会 fail closed 并开告警，不可能写错数据；无赛事马场纳入后无实际抓取）；"不等 107 闭环"在换绑路径已上线前提下安全。发现 V5-1（P1，106/107 闭环断言与背景段矛盾且无证据）、V5-2（结构级 proof 计数 5≠8 不一致）、V5-3（背景段生产版本过时）、V5-4（福島补证与 discovery 窗口同日竞速）。

## 第 6 轮（2026-09-27，同一独立审阅者）：APPROVED

V5-1~V5-4 全部闭合：106/107 证据以生产只读快照入库（SHA dd16f5ae…38b 逐字符一致）、结构级证据包入库（SHA e2816af9…f45 一致）、背景版本更新为 15c79f22、福島补证加硬时序约束。残留 3 条 P3 措辞建议已落实（107 表述笔误、proof 解析方法注明）。

代码审阅（第 4-5 轮之间完成）：PR #219（T/F 归一）与 PR #221（换绑路径）APPROVED；4 条 P2 加固（行锁、rebind_lifecycle_missing、replay 幂等、rebind_selection_lost）已实施复验，144 项相关测试通过。

**计划整体状态：APPROVED，进入实施。** 仍不可豁免的实施前核对项：服务器运行态（.env/锁/release_0079 模块）、10/1 CNAME 级 proof、新激活脚本的独立审阅。
