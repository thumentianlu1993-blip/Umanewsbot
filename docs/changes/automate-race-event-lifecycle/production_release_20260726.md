...
- 生产 revision：`ef54a1836dd1fe1840f2d4765ebb73a1d130c645`。
- `web / worker / beat` 镜像：
...
- 决策结果：`transition=7 / noop=28 / error=0 / ineligible=0`。
...
- shadow 前必须取得可信 `race_datetime`、冻结逐场 enrollment/baseline manifest，
  完成区域观察策略并取得针对精确赛事集合和观察窗口的独立用户授权。
- 生产发现的双重 migration 执行入口必须在下一个含 migration 的部署前独立修复并 review。

## 7. 后续状态补充

- The Racing API schema v2 proof runner 已由后续 PR `#27` 修复并合入
  `origin/main@a59956b327157d29630fab1f1c98ba9c9cacfed0`。
- 该代码修复不等于新增 provider 联网、shadow、赛事纳管或业务数据写入授权。
- Sporting Life、ZEturf、HRN 的赛后内部参考源规划属于阶段 B0.1，不改变本记录的阶段 A 生产事实。
