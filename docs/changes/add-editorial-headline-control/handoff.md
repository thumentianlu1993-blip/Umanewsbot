...
- 方案审核通过后必须停止；
- 用户明确回复“确认实现”“开始实现”或同义授权前，不得写测试、实现、迁移或启动实现 subagent；
- 最新成功代码 review 后仍需针对当前版本取得单独发布授权；
- 发布授权前禁止 commit、push、PR、merge、部署、数据库迁移、服务重启和生产写入。

用户本轮说明将项目交给 Claude 实现，但本轮直接动作是“编写交接文档”，并没有让当前 Codex 开始测试。
为避免把交接意图误当成已消费的实现授权，Claude 在写第一行测试前应确认用户已在其会话明确说出
“确认实现”“开始实现”或等价指令。若没有，先用一句话请求确认，不得自行越过。
...
- 禁止使用任何 OpenSpec skill 或 OpenSpec CLI；
- 不修改 `AGENTS.md`；
...
如果 Claude 会话中尚无用户明确“确认实现/开始实现”或等价指令，先停下请求确认。不要把本交接文档本身
当成自动实现授权。
