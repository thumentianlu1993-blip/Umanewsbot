...
本轮完成方案、来源研究及独立审查。以下代码任务均未执行。按根 AGENTS.md 在相同范围内推进；发布/数据写入不由方案审核代替授权。
...
- [ ] (application) 加 `RaceEventIdentityKey`、`RaceDataSyncSourceBinding` 和 Enrollment v2字段；仅 additive migration，约束和默认兼容验证。
...
- [ ] (integration) 实现v2 policy、route-local digest、冻结legacy resolver；无关新增route不得改变旧准入。
- [ ] (application) 实现登记/attach/CAS、分能力粘滞选源、连续故障/circuit后的有期限备用授权及receipt；共享admission覆盖状态、资料写入和公开读。
- [ ] (integration) 修改selector/task/provider分派为binding驱动，一个event单claim、单claim单source；所有末端writer禁用隐式TRA/source FK假设。

## 2. 来源适配（每项测试在实现之前）

- [ ] (integration) JRA：T01/T11–T16真实fixture RED；提取既有parser，赛前官方ID→登记→真实结果链接→完整结果、显式finality GREEN。
...
- [ ] (application) 在 `.env.example` / `settings.py` 加三开关及v2 policy schema接入，全部默认关闭；维护README/运行文档相关入口。
