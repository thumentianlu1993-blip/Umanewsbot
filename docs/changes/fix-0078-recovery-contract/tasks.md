...
- [ ] (operations) 配置不含生产.env的Linux/PG16隔离环境；明确测试端口、库、卷和网络边界。
- [ ] (operations) 重现相关基线失败，记录名称/原因；不声称本地源码节选能够跑全套。

## 2. RED

- [ ] (application) T02–T11：migration graph、catalog、不可逆、锁与原子性测试。
- [ ] (integration) T12–T22/T28/T35–T42：三类入口备份检查与每个stop/closed/marker/completion断点的真实resume测试。
- [ ] (operations) T23–T27：真实拒绝策略和显式代际fixture；防回归负例。
- [ ] (operations) T29–T32：隔离备份恢复、失败和队列恢复边界。
...
- [ ] (operations) 更新部署/恢复文档，明确旧世代恢复边界和备份数据损失窗口。

## 4. 验证与交付准备

- [ ] (application) 相关单元和shell套件全绿。
- [ ] (integration) PG16迁移、锁竞争、失败重放、备份恢复全部通过。
- [ ] (operations) T33检查、全量stable同环境基线对比，无新增失败。
- [ ] (operations) 独立代码review及复审。
- [ ] (operations) 按rollout准备精确发布包与停止/恢复路径；更新必要状态文档。
