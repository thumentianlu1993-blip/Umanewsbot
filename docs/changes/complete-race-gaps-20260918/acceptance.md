...
- JRA103/104/105赛时及12/13/11条名单在两域名公开，105保持“参赛名单，马号待公布”。JRA开关在此前d0bb40fb配置发布启用，本次7e111914四应用继续保持；3h/1h/10min和预算不变。103于04:27 UTC自然成功刷新，证明一次1h周期；104/105的3h和比赛日10min自然周期尚未据此证明。
- 另9场空赛时与77条已审核候选入库并重放通过：491、492、969、770、771、772共45条，494/495共20条，191共12条。PR205应用上线后，两个域名18页全部77条名单（双域名共154行）逐字段核对通过，完整北京/当地日期时间和官方/参考标签一致。
...
当前未来范围22=JRA3+已公开人工核验候选9+未补齐10。JRA窄路径不代表NAR/海外持续自动补卡已实现。原表额外9条旧scheduled记录均有active canonical映射，已排除重复缺口；未逐条验证其HTTP重定向与目标内容。
...
05:20 UTC发布后，四应用实际7e111914、镜像8f311074，配置/JRA启用保持；八服务运行，配置了健康检查的服务均healthy，两个worker响应，普通/v2队列0，旧LIVE7543未消费。Django check、空迁移计划、备份及恢复凭据、锁收尾通过。129历史页面和JRA6页再次复验无回退，见[发布记录](release.md)。

以下为发布前历史采样：04:18 UTC八服务running、restart=0、OOM=false，web/db/redis healthy；四应用同677fe69f镜像、web实际d0bb40fb/JRA=true，DB只读SELECT通过。队列celery18、race_sync_v2=0、旧race_live7543；这是单次采样，未据此宣称队列全链无积压。采样时部署锁已释放。103下一due04:23，104/105为06:23；该时点尚未自然tick验收。
