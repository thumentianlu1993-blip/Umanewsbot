...
| stable 测试基线 | 生产基线 274 个唯一失败 ID；#187 独立候选消除 31 个且无新增。#188 六模块候选消除 104 个原失败 ID、另有 3 个新增，已修正并更新为 `cb829735` 待 CI；外连接加锁 #189 和性能 #190 均已实现、独立审核并提交 Draft，待 Linux 全量验证 | 先核对 #188 的新增失败是否清零，再验证 #189/#190；继续处理其他夹具和合同簇。各候选分别对比生产，不能直接相加为主线结果。既定关闭能力的测试对齐现行合同，不能靠删测试、降低断言或批量 expectedFailure 掩盖 |
...
| 审核工具和本机可复现性 | PR #187 与 #188 首轮 Linux CI 已产出一致的正式前后指纹；Windows 缺少 O_NOFOLLOW/部分 dir_fd 支持，另有文件时间字段差异，补充快照不冒充正式通过 | 受支持路径为现有 Linux/PG16 CI 的固定提交验证，构件中保留 review-fingerprint-before/after；本机仅作快速反馈和明确限定的补充取证。继续保留符号链接/reparse、身份变化和读写边界检查，不降级安全读取 |
...
原 PC 未提交文件不能从 PR 状态推断已恢复；目录恢复面仍需单独核对。
