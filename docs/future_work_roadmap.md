...
| 新一批测试/并发修复 | #195 `be54a04a` 官方发布夹具与临时赛果恢复行锁、#196 `6d19771f` 纳管 control 并发新建拒绝，均 Draft，独立审核通过、PG CI 待核验；#194 全量另暴露今日到期的旧 full-cohort 夹具 | 优先完成 #194 交付；再核验两候选，最小修复到期夹具并保留真实授权过期拒绝，不计入尚未运行的组合结果 |
| 审核工具和本机可复现性 | #187–194 当前已核验候选的 Linux CI 提供一致的正式前后指纹；Windows 缺少 O_NOFOLLOW/部分 dir_fd 支持，补充快照不冒充正式通过 | #195/#196 等新候选仍需自身 Linux/PG16 逐构件核验；保存正式指纹及文件身份、符号链接和读写边界检查。原生进程中断须先核实终止，再恢复原审核上下文 |
