## 2026-09-20 赛事自动化设计更新（尚未实施）

用户新增明确要求：所有地区任一可信来源命中即可登记，同一赛事多源去重。[多来源登记方案](changes/multisource-race-enrollment/PLAN.md)经独立 Agent 两轮审核通过，作为下一步实施入口。下文旧提案中“仅完整能力来源竞争整场纳管／仅赛果来源只能更正”的要求，在本次新实现中由“登记与能力分离、单 owner、多 binding、逐能力粘滞及受控故障切换”取代；下文日期、旧验收和生产事实保留为历史，不能据此宣称新链路已生效。具体身份、迁移、测试及恢复合同以新方案为准。

# 后续工作路线图与 Agent 交接

> 2026-09-17 补充：赛事区新发现问题的实施范围统一见[最小修复计划](changes/fix-race-section-gaps/PLAN.md)，已纳入用户要求的 D−4 起持续更新和提频，以及限定 JRA 赛前读取；状态为主线程修订／固定子代理两轮审核通过、未实施，不扩大下方历史阶段的完成声明。

> - 状态日期：2026-09-15；应用主线合并 `7981439555bec747d462910506c56b08449ae621`，生产候选 `9a88243f`。
> - 当前统一交接：[9月6日至15日工作交接](changes/release-pr201-production/handoff.md)。
> - 以下旧日期记录保留历史；生产状态仍以当次只读核验为准。

## 0. 当前交接状态（2026-09-15）

用户本轮已要求上线PR201并准备交接；剩余旧失败仅在证实影响线上时修复，其他跳过。M3–M6仍暂停，历史失败新闻不处理。

| 事项 | 当前状态 | 接手后的处理 |
| --- | --- | --- |
| PR201第二批整合 | 已合并并部署，四应用一致、0078空计划、六场赛果及恢复凭据验证通过 | 从生产`9a88243f`与当前main重新建立基线；不重复发布旧候选 |
| 剩余45条旧测试失败 | 本轮线上核查未见需补修的实际影响；测试数据/接口、旧文案、历史工具缺失等保留证据并跳过 | 仅在出现实际线上影响时按具体路径修复，不为清零重写旧测试或恢复旧界面 |
| 已知TRA回执升级字段长度 | 代码缺陷已知，现有5次首次导入成功，近期无该升级路径运行/错误证据 | 暂不执行或扩量；若准备启用该路径或出现真实调用失败，再处理 |
| M2新赛事自然闭环 | 962/963已有正式版本、公开9/11行、同赛果自然correction证据 | 上游明确正式确认标记及完赛后30分钟确认尚未证明；整体不标通过。原只读任务持续观察 |
| M2赛时/旧赛果/发现诊断 | #183恢复956；#184版本标记与发现诊断；#194当地赛时已发布并验证 | 保留历史证据，按当前源状态判断新问题；不重放历史修复 |
| 490/766发现未匹配 | 截至9月10日01:40 UTC，490为response_empty；766出现course_not_found / local_date_not_found | 分桶诊断不等于根因已解决；后续结合来源日期/场地证据判断，不放宽身份规则或额外调用来源 |
| 历史PR与本地目录 | 13个来源PR随#201进入主线；旧#193及7个历史PR保留；2个旧worktree共7份未提交文档保留 | 不将旧工作区/文档快照覆盖当前状态，不自动删除或盲合 |
| 历史工具源码与原PC候选 | 两个runner源码缺失对应9条测试；原PC candidate-only目录未证实恢复 | 本轮跳过，保留来源恢复线索，不重建猜测实现 |

已完成：M1实现/启用、755/756/757恢复、0078恢复合同与开关保留、956公开恢复、发现诊断及镜像标记、当地赛时修复、第二批测试/并发/性能修复生产交付。

保护边界：旧`race_live=7543`、人工锁、现有业务开关及普通代码rollback禁用保持；未经新范围授权不扩量抓取、不批量改生产数据、不处理历史失败新闻。通用人工门禁仅见根AGENTS.md。
