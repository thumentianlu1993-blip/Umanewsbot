...
| stable 测试基线 | 生产基线为 274 个唯一失败 ID；PR #187 独立候选减少 31 个，PR #188 首轮独立候选减少 19 个，均无新增。#188 已更新为六模块候选 `61afbf59`，新 CI 待完成 | 继续验证剩余事务修复，并处理已暴露的外连接加锁错误、性能波动及夹具簇；候选数字各自对比生产基线，尚不是主线集成结果。既定关闭能力的测试对齐现行合同，不能靠删测试、降低断言或批量 expectedFailure 掩盖 |
