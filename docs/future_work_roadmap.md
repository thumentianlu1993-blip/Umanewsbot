...
| stable 测试基线 | 生产基线 274 个唯一失败 ID；独立候选 #187 消除 31 个、#188 消除 104 个、原 #189 消除 16 个、#190 消除性能失败 1 个、#191 消除 10 个，均无新增。#189 已纳入 #188 解决重叠冲突，组合 `22ab56b8` CI 待验证；#191 另有 6 个测试暴露后续断言失败 | 核对 #189 组合结果，继续处理 #191 后续断言及其他旧合同/夹具簇；各候选分别对照生产，不能相加为主线结果。禁止通过删测试、降低断言或批量 expectedFailure 掩盖 |
...
| 审核工具和本机可复现性 | #187、#188 当前、#189 原独立候选、#190、#191 的 Linux CI 已产出一致的正式前后指纹；Windows 缺少 O_NOFOLLOW/部分 dir_fd 支持，补充快照不冒充正式通过 | 受支持路径为现有 Linux/PG16 CI 固定提交验证；构件保留正式指纹。新 #189 组合候选单独验证；继续保留符号链接/reparse、身份变化和读写边界检查 |
