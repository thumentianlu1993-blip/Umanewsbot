...
> - 当前主线：`ced88882152729c8e31a9cddb5fd83094e30d196`；生产候选：`51a46c37`
...
| M2 当地赛时显示（当前优先） | #194 已按用户批准包合并并部署固定 `51a46c37`，77 项相关、45 项发布、独立审核及生产部署复验通过；全量旧失败保留 | 截至 `2026-09-10 05:35 UTC`，962/963 均已由自然同步补齐为英国当地 15:00/15:35（北京时间 22:00/22:35），双域名四页一致。原拒绝审计保留，同 observation 分别只新增一次 applied；两场后续相同来源内容的 race_time 同步均成功且未重复写入。本次赛时显示及重放验收通过；未强制抓取或手工回填，M2 正式赛果/公开/correction 整体验收仍未完成。 |
...
| stable 测试基线 | 新生产 #194 CI 为 275 个唯一失败（旧 274 加未改动夹具到期 1）；下述独立结果仍与旧生产 69955960 比较；#193 `9ebdf657` 的 #186–191 组合 CI 已核验，实际消除 163 个、剩余 111 个、无新增。独立 #192 当前 `de5fb540` 剩余 235 个，38 个发布相关失败消失，另 1 个未改动性能用例本轮通过；相关模块及三项新增 PG 回归通过 | 继续处理其他旧合同/夹具簇，后续整合须重新核验实际组合；#193 不含 #192/#194，不能将独立修复数相加。禁止删测试、降低断言或批量 expectedFailure 掩盖 |
...
| 新一批测试/并发修复 | #195 相关 31 项、#196 相关 43 pass/2 条 macOS skip 及实际 PG 并发通过；#197 相关 2 项、#198 26 项、#199 两模块 30 项加 2 条保护测试通过，各自 45 项发布专项与正式指纹一致；全量仍失败 | #198 最后 comparison job 尚待结束；后续整合需以新 main 重新核验实际组合，不累计独立候选计数。#195–199 均未合并部署 |
| 审核工具和本机可复现性 | #187–199 已核验候选的 Linux 发布构件提供一致正式前后指纹；Windows 缺少 O_NOFOLLOW/部分 dir_fd 支持，补充快照不冒充正式通过 | 后续新候选和整合仍需自身 Linux/PG16 验证。保持原生只读审核；进程中断先核实终止，再恢复原上下文 |
