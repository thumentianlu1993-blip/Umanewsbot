...
| 490/766 发现未匹配归因 | PR #184 已部署诊断，具体原因待现有自然任务证据 | 读取已产生的诊断并区分来源缺数、日期/场地/名称差异、权限/覆盖或实现缺陷；可执行代码缺陷补测修复，外部限制保留证据，不能以放宽身份规则伪造命中 |
| stable 测试基线 | 最新 Linux 候选 4,943 项，274 个唯一失败 ID；与前生产失败集合相同 | 按根因复现并修复，先处理共享导入路径、事务/锁与过期夹具；活跃范围无未解释失败。既定关闭能力的测试对齐现行合同，不能靠删测试、降低断言或批量 expectedFailure 掩盖 |
| 文档合同与状态漂移 | 工作分支已修复 5 个文件的旧流程引用并刷新当前状态；原检查器及 4 项合同测试通过，待独立审核和交付 | 更新当前状态和历史边界，保持原校验规则；审核后提交交接文档。新闻恢复、0078、956 和 PR #184 的已完成事实有明确证据 |
| 审核工具和本机可复现性 | POSIX 指纹工具在 Windows 缺少 O_NOFOLLOW，现有补充快照有明确限制 | 核对受支持执行路径并提供可复现的正式审核指纹证据；如需代码修复，保留符号链接/reparse、身份变化和读写边界检查，不降级为不安全文件读取 |
| 历史 PR / worktree 交接 | 2026-09-09 再查仍有 #87/#85/#59/#49/#45/#26/#24 共 7 个 open PR；原 PC 两个 candidate-only 目录尚未证明可在本机获取 | 按当前主线做只读分类：仍有价值、被替代、需核心判断、缺少原始文件。保存必要差异和处置建议；不把大分支整包合入，不批量关 PR 或删目录 |
