...
> 更新：2026-09-26。统一交接为[跨任务工作交接（9月6日至26日）](changes/release-pr201-production/handoff.md)，此链接沿用原交接文件。9月15日原文保留为历史附录。生产当前事实以current_state及带时间的只读证据为准。

## 0. 当前交接状态（2026-09-26）

已核对本项目其他任务、近期PR、旧交接、本机采集账本及当前生产样本。远端主线为PR213文档合并82556fd1，生产候选26706602/0079；文档SHA与应用SHA分开。详细证据与限制见统一交接，不以旧任务标题判定运行状态。

| 工作 | 已完成 | 仍待进行 |
|---|---|---|
| 赛事自动化M1与旧恢复 | #176–202主链、755/756/757及956恢复、赛时/发现诊断、PG并发与发布恢复 | M2新赛事全链与完赛30分钟正式确认SLA未完整补证 |
| 赛前资料/历史补齐 | #203–206 D−4框架、129/132历史缺口1209行、9场77条审核名单 | 828/830缺正式结果，924暂定无有效公开授权；海外新赛果入口仍有空缺 |
| 展示与持续刷新 | #207–211等级/距离/译名归一化、北京时间、名单/赔率/退赛持续刷新、超长URL修复；两轮自然样本已通过 | 所有地区/时段、长期时延及来源访问限制未整体验收，不能把预览卡当赛后登记 |
| 多来源登记 | #212/#213已上线，0079、单event多binding、强身份去重、赛后补入；JRA三场12/13/11正式公开并持续刷新 | **优先补106/107的source_identity_missing**；其他地区逐来源proof/准入和完整自动发现 |
| 马匹资料采集 | 原试点与批1–8处理；当前批1–9去重4027核心候选、37缺口（不含原10试点） | 批9暂停464/500，36待处理；原14→15登记差异归因，重新绑定release后再按恢复指令继续；不等同正式入库/完整生涯 |
| 旧测试/源码/PR | 全量固定候选无新增失败，67发布合同通过；8个open PR已重新列出 | 保留45历史失败；只修有实际影响项；旧PR分类处理和原PC候选恢复，禁止盲合/清理 |
| 长期M3–M6 | M3的北京时间、展示归一化、刷新专项已获单独授权并交付 | 剩余字段/非完赛历史覆盖、新闻联动、马匹质量、新地区、模型评估/移动后台仍保留，不自动扩大实施范围 |

建议接手次序：新赛事身份发现 → 海外赛前/赛后衔接及旧正式结果 → M2验收/策略续期 → 马匹采集恢复与身份审核 → 其余产品质量和长线项目。当前诊断样本是有界样本，不代表全站完整率；本次仅更新文档，没有重新部署或恢复采集。

### 0.0 取代旧方案的规则

2026-09-20方案已随PR212实施，不再是“尚未实施”：任一已准入来源凭强身份可登记，登记与能力分离，单owner、多binding；仅结果能力也能经受审证据链冷启动。名字/翻译/拼写相似只用于候选召回，强身份和唯一约束决定自动去重。生产当前真实多来源策略只覆盖JRA阪神/中山result；通用七地区合同不等于七地区实网覆盖。旧“只有完整能力来源可竞争纳管”只解释v1历史。
...
3. **来源可信与登记、资料能力分开。** 任一已准入来源提供唯一强身份即可登记；单event/owner下允许多来源binding，资料按能力粘滞和受控故障切换。来源可信不等于已经具备当前地区实网proof，TRA、官方、Racing Post、ATR等都需验证具体路线。v1既有登记兼容保留，不因新策略自动批量转换。
...
当前细目、证据时间与完成标准见[统一交接第4节](changes/release-pr201-production/handoff.md#4-当前仍待进行按优先级接手)。不要重新执行已完成的M1和755/756/757恢复，不把旧candidate-only迁移0079复制到当前已发布0079。

| 工作 | 当前状态 | 接手边界 |
|---|---|---|
| 新赛事来源身份 | 106/107未登记，海外491/492/494/495无正式结果 | 补发现入口及真实proof，严格强身份，验收自然闭环 |
| 官方结果与M2 | 828/830无正式revision，924仅provisional；30分钟SLA未补证 | 不伪造确认、不重启已停止的旧复核任务来刷成功数 |
| 马匹采集 | 本地批9暂停464/500，自动化PAUSED | 当前停点取代旧21/200研究快照；保留原用途/账本，未经恢复指令不续跑 |
| P0正式补全 | 原32/180/230候选及16blocker是历史记录，未证实本轮apply | 重验candidate、身份/冲突及生产基线后再交付 |
| 历史PR/源码 | #193及7个历史PR仍open；原PC两份未提交candidate未证实恢复 | 不批量关闭，不重建猜测源码，不整分支盲合 |

原PC候选目录名stabilize-race-data-pipeline、racing-completion-orchestration只作为恢复线索，不是本机存在或远端已保存的证明。原horse_data脏树还有未入main的研究资料，应单独核对保存范围。

## 6. 历史产品决定（2026-09-05；第一项由PR212新规则取代）

以下五项已于 2026-09-05 由用户审核定稿；第 1、4 项否决了工程建议并按用户口径改写，方案文档已同步修订。
完整决定文本与后果见 [关键决策](decisions.md)。

1. **历史v1规则，已由第0.0节多来源能力分离取代**：所有可信来源按“先到先得”竞争首次纳管；纳管权只授予一次、同轮平局按
...
这描述当时的实施起点；M1及后续PR212已上线，不能再从旧RED阶段重建一套实现。

## 7. 长期产品清单（2026-09-26状态修订）

### 7.1 赛事数据、状态与赛果

| 待修事项 | 建议方案 | 完成标准 | 依赖/优先级 |
| --- | --- | --- | --- |
| 赛前持续刷新：框架已上线、全覆盖待验 | 沿用#203/#209/#210；继续验证各来源开放窗口、退赛/骑师/档位及长期节奏 | 保留已通过492/969自然样本；新增来源须取得自己的字段与周期证据 | M2/M3，P1 |
| entry/result映射：已审路径有约束、全来源/历史覆盖待验 | 明确拆分 `entry_status` 与 `result_status`，定义状态映射和不变量；先模型/接口设计与 migration 评审 | 退赛/未出赛不会被误当完赛；历史兼容和回滚通过 | M1/M3，P0 |
| 非完赛保留：已审路径已修，历史缺口仍待补 | 以 declared/actual starter 守恒生成结果；保留 scratched、withdrawn、non-runner、DNF、PU、DSQ 等稳定 reason code | 每个实际参赛者恰好一条结果或非完赛状态；未知状态 fail closed | M1，P0 |
...
| 字段归一化：等级/距离/译名展示已上线，其他字段/历史覆盖待补 | 分字段保存来源证据，分批补齐 distance、age restriction、surface、course、grade；冲突进 review | 每字段有来源/时间/hash；无证据不覆盖已有可信值 | M3，P1 |
| 北京时间展示已上线；新来源时间完整性继续验收 | 复用已发布aware时间/DST/北京展示；缺时间仍保留缺口 | 不重复建设；新地区/延期来源提供独立验证 | M2/M3，P1 |
| 赛果复核来源route仍有缺口 | 复用已发布多来源登记，补具体identity/route/proof；旧复核任务保持停止 | 候选不再静默消失；`route_missing` 有可行动原因 | M1，P0 |
...
| 非完赛中文展示：部分路径已交付，历史QEII等待专项验收 | 为退赛、竞走中止、失格、未完赛提供中文稳定显示，不用空名次代替 | QEII 等历史 fixture 显示正确，来源证据不泄露内部字段 | M3，P1 |
...
## 8. 分阶段 Roadmap（原设计与当前执行边界）

M0/M1核心与M2发布动作已经完成，M2全范围验收未完成；M3部分专项已上线。以下阶段设计用于检验剩余能力，不能把旧预计“无migration”、v1完整能力限制或RED起点作为当前待执行命令；当前schema为0079，策略以第0.0节为准。
...
- 给当前 8 个 open PR 分别标记：继续、需要 rebase/拆分、已被替代、待归档；归档/关闭另行执行。
