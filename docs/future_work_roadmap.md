...
| stable 测试基线 | 生产基线 274 个唯一失败 ID；#189/#188 组合 `22ab56b8` 剩余 160 个、消除 114 个且无新增。独立 #187 消除 31 个、#190 消除性能失败 1 个、#191 当前 `151e2e92` 消除 17 个（7 项映射通过）；#192 的 `d68b4258` 消除 36 个，均无对生产新增。#192 当前 `de5fb540` 修复其后两条发布错误，CI 待核验。#193 `9ebdf657` 整合 #186–191 的 27 个已审文件，45 项发布专项通过，全量组合待完成 | 核对 #192 新候选及 #193 组合的实际 CI；继续处理其他旧合同/夹具簇。各候选分别对照生产，不能相加为主线结果。禁止通过删测试、降低断言或批量 expectedFailure 掩盖 |
...
| 审核工具和本机可复现性 | #187、#188、#189 当前组合、#190、#191 当前候选的 Linux CI 已产出一致的正式前后指纹；Windows 缺少 O_NOFOLLOW/部分 dir_fd 支持，补充快照不冒充正式通过 | 受支持路径为现有 Linux/PG16 CI 固定提交验证；构件保留正式指纹。#192 及后续修复仍需单独验证；继续保留符号链接/reparse、身份变化和读写边界检查 |
