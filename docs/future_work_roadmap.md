...
| M2 当地赛时显示（当前优先） | #194 `51a46c37` 的 Linux/PG 相关模块 77 项、发布合同 45 项及独立审核通过；全量 275 个唯一失败，其中额外 1 项为未改动旧夹具今日到期，已隔离复现，CI 仍红。02:35 UTC 双域名仍显示待定，生产未变 | 精确发布包待交付审核；上线后观察自然同步补齐 962/963，并核验当地时间 15:00/15:35。不手工回填或触发同步 |
...
| 新一批测试/并发修复 | #195 相关 31 项与发布 45 项、#196 原/新两项 PG 并发及相关 43 pass/2 条 macOS 条件 skip、发布 45 项已核验，正式指纹一致；全量仍有失败。#197 固定查询数时钟、#198 实时赛果夹具已 Draft 且独立审核通过 | 优先完成 #194 交付；核验 #197/#198 自身 Linux/PG CI，不将独立候选结果累计为组合已通过。公开说明及两份验证文档已获明确授权并上传 |
| 审核工具和本机可复现性 | #187–196 当前已核验候选的 Linux CI 提供一致的正式前后指纹；Windows 缺少 O_NOFOLLOW/部分 dir_fd 支持，补充快照不冒充正式通过 | #197/#198 等新候选仍需自身 Linux/PG16 逐构件核验；保存正式指纹及文件身份、符号链接和读写边界检查。原生进程中断须先核实终止，再恢复原审核上下文 |
