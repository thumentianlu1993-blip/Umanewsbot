...
| stable 测试基线 | 生产基线 274 个唯一失败 ID；#189/#188 组合 `22ab56b8` 经 CI 核验剩余 160 个、消除 114 个且无新增。独立候选 #187 消除 31 个、#190 消除性能失败 1 个、#191 当前 `151e2e92` 消除 17 个，均无对生产新增；#191 的 7 项映射用例全部通过，历史对照仅因 #190 覆盖的旧性能用例标红；#192 `9fef94fd` 的发布测试时钟/日期夹具已审，Linux CI 待核验 | 核对 #192 当前 CI，继续处理其他旧合同/夹具簇；各候选分别对照生产，不能相加为主线结果。禁止通过删测试、降低断言或批量 expectedFailure 掩盖 |
