...
def _parse_detail_page(body: bytes, *, source_url: str):
    import sys
    server_root = str(Path(__file__).resolve().parents[2] / "server")
    if server_root not in sys.path:
        sys.path.insert(0, server_root)
    from stable.race_reference_parsers.jra import _parse_detail_page as parse_modern
    try:
        return parse_modern(body, source_url=source_url)
    except RuntimeError:
