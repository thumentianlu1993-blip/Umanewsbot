...
class NewRegionProbeContractTests(TestCase):
    ...
    def _assert_common_probe_schema(
        self,
        result: dict,
        *,
        expected_technical: str = "not_probed",
    ):
        required = {
            "source_key",
            "http_status",
        ...
        self.assertEqual(result["technical_status"], expected_technical)
        self.assertRegex(result["artifact_sha256"], r"^[0-9a-f]{64}$")
        self.assertTrue(result["reviewed_at"])

    def test_adapter_declared_unknown_cannot_override_canonical_blocked(self):
        result = self._run_probe(_ProbeAdapter)
        self._assert_common_probe_schema(result)
        self.assertEqual(result["automation_permission_status"], "blocked")
        self.assertEqual(result["effective_production_status"], "production_blocked")
        self.assertEqual(result["deferred_reason"], "technical_access_blocked")
        self.assertEqual(result["request_count"], 0)
        self.assertEqual(result["parse_quality"]["list_count"], 0)

    def test_adapter_declared_approved_cannot_override_canonical_blocked(self):
    ...
    def test_human_output_prints_full_contract_for_success_and_error(self):
        for adapter_class in (
            ...
            with self.subTest(adapter=adapter_class.__name__):
                ...
                self.assertIn("technical_status=not_probed", output)
                self.assertIn("automation_permission_status=blocked", output)
                self.assertIn(
...
class ModeOffCandidateGateTests(TestCase):
    def test_source_scoped_candidate_keeps_source_region_and_blocks_publish_and_qq(self):
        ...
        self.assertEqual(
            qq_decision.reason,
            "source_public_distribution_blocked",
        )
...
class NewRegionQQGateTests(TestCase):
    ...
    def test_explicit_subscription_all_public_and_high_value_are_independent_gates(self):
        blocked_canada = self._public_article(
            region=RacingRegion.CANADA,
            source_site=SourceSite.WOODBINE_NEWS,
        )
        blocked_united_states = self._public_article(
        ...
        self.assertEqual(
            should_push_news_to_qq(blocked_canada, target=target).reason,
            "source_public_distribution_blocked",
        )
        self.assertEqual(
            should_push_news_to_qq(blocked_united_states, target=target).reason,
            "source_public_distribution_blocked",
        )
    ...
    def test_empty_subscription_defaults_to_japan_and_manual_lock_is_independent(self):
        ...
        blocked_canada.attribution_summary = {
            "review_candidate": {"primary_region": RacingRegion.CANADA}
        }
        blocked_canada.save(update_fields=["attribution_summary", "updated_at"])
        japan = self._public_article(
            region=RacingRegion.JAPAN,
            source_site=SourceSite.JRA,
            locked=False,
        )
        japan.attribution_summary = {
            "review_candidate": {"primary_region": RacingRegion.JAPAN}
        }
        japan.save(update_fields=["attribution_summary", "updated_at"])
        canada = self._public_article(
            region=RacingRegion.CANADA,
            source_site=SourceSite.JRA,
        )
        ...
        self.assertEqual(
            should_push_news_to_qq(japan, target=target).reason,
            "region_review_required",
        )
        japan.attribution_locked = True
        japan.save(update_fields=["attribution_locked", "updated_at"])
        self.assertTrue(should_push_news_to_qq(japan, target=target).allowed)
        self.assertEqual(
    ...
    def test_adapter_and_probe_preserve_403_and_429_metadata(self):
        ...
        for source_key, status_code, final_url in cases:
            ...
            with self.subTest(source_key=source_key, layer="probe"):
                ...
                def budgeted_failure(url, **kwargs):
                    before_transport_get = kwargs.get("before_transport_get")
                    if before_transport_get is not None:
                        before_transport_get(
                            kwargs.get("request_kind", ""),
                            url,
                        )
                    raise failure
