...
def _validated_https_url(url: str, allowed_hosts: set[str]) -> str:
    ...
    try:
        ...
        raise ValueError("bounded_http_invalid_port") from exc
    if port not in {None, 443}:
        raise ValueError("bounded_http_port_not_allowed")
...
def get_bounded_html(
    ...
    *,
    ...
    request_kind: str = "",
    before_transport_get: Callable[[str, str], None] | None = None,
) -> BoundedHtmlResponse:
    ...
    while True:
        if before_transport_get is not None:
            before_transport_get(request_kind, current_url)
