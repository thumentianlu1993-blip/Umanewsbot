...
RACE_CALENDAR_CURSOR_SALT = "stable.public-race-calendar.cursor.v2"
RACE_CALENDAR_CURSOR_VERSION = 2
...
def encode_race_calendar_cursor(event, *, filters: dict[str, str]) -> str:
    ...
    public_date = stamp.day
    public_start_time = stamp.clock
    payload = {
        "v": RACE_CALENDAR_CURSOR_VERSION,
        "filters": race_calendar_filter_fingerprint(filters),
        "key": {
            "date_is_null": int(public_date is None),
            "public_date": public_date.isoformat() if public_date else None,
            "time_is_null": int(public_start_time is None),
            "public_start_time": (
                public_start_time.isoformat() if public_start_time else None
            ),
            "event_id": event.pk,
        },
    }
...
def decode_race_calendar_cursor(
    ...
    *,
    ...
    try:
        ...
        key = payload["key"]
        public_date = (
            date.fromisoformat(key["public_date"])
            if key.get("public_date")
            else None
        )
        public_start_time = (
            time.fromisoformat(key["public_start_time"])
            if key.get("public_start_time")
            else None
        )
    ...
    if cursor.date_is_null not in {0, 1} or cursor.time_is_null not in {0, 1}:
        return None
    if cursor.date_is_null != int(cursor.public_date is None):
        return None
    if cursor.time_is_null != int(cursor.public_start_time is None):
        return None
...
def race_calendar_cursor_filter(
    ...
    *,
    ...
    if cursor.public_date is None:
        date_prefix = Q(public_date__isnull=True)
        if not after:
            condition |= Q(public_date__isnull=False)
    else:
        date_prefix = Q(public_date=cursor.public_date)
        if after:
            condition |= Q(public_date__gt=cursor.public_date)
            condition |= Q(public_date__isnull=True)
        else:
            condition |= Q(public_date__lt=cursor.public_date)

    if cursor.public_start_time is None:
        time_prefix = date_prefix & Q(public_start_time__isnull=True)
        if not after:
            condition |= date_prefix & Q(public_start_time__isnull=False)
    else:
        time_prefix = date_prefix & Q(
            public_start_time=cursor.public_start_time
        )
        comparator = (
            {"public_start_time__gt": cursor.public_start_time}
            if after
            else {"public_start_time__lt": cursor.public_start_time}
        )
        condition |= date_prefix & Q(**comparator)
        if after:
            condition |= date_prefix & Q(public_start_time__isnull=True)
...
def public_default_race_date_window(queryset, *, today: date) -> DefaultRaceDateWindow:
    ...
    只承认有 ``public_date`` 的实际比赛日，发出恰好两条有界聚合查询：

    - ``public_date <= today`` 按日期倒序最多 11 个 distinct 日期，并以
      ``Min("id")`` 聚合出该日代表赛事 ID；
    - ``public_date > today`` 按日期升序最多 11 个，同样带代表 ID。
    ...
    dated = queryset.filter(public_date__isnull=False)
    before_rows = list(
        dated.filter(public_date__lte=today)
        .values("public_date")
        .annotate(representative_id=Min("id"))
        .order_by("-public_date")[:RACE_DATE_WINDOW_LIMIT]
    )
    after_rows = list(
        dated.filter(public_date__gt=today)
        .values("public_date")
        .annotate(representative_id=Min("id"))
        .order_by("public_date")[:RACE_DATE_WINDOW_LIMIT]
    )
    if before_rows and before_rows[0]["public_date"] == today:
        anchor = today
    elif after_rows:
        anchor = after_rows[0]["public_date"]
    elif before_rows:
        anchor = before_rows[0]["public_date"]
    else:
        return DefaultRaceDateWindow(anchor=None)
    before_desc = [row["public_date"] for row in before_rows if row["public_date"] != anchor]
    after_asc = [row["public_date"] for row in after_rows if row["public_date"] != anchor]
    ...
    for row in [*before_rows, *after_rows]:
        representative_by_date.setdefault(row["public_date"], row["representative_id"])
